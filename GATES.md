...
| **P1.1** | An English browser reaches an LTR login screen before auth resolves | | PENDING |
| **P1.2** | The manual switch overrides detection and survives a refresh | | PENDING |
| **P1.3** | `profiles.locale` exists, is per-person, and `NULL` still means "let the browser decide" | | PENDING |
| **P1.4** | `npm run build:almoni` still stamps `data-font-mode="almoni"` | | PENDING |
