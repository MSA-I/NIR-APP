...
- **מיגרציות/בדיקות:** סקר את `0086`–`0097` ואת P16–P25; לא יצר migration עצמאי.
...
- **קבצים/מיגרציות:** `supabase/migrations/0086_*` עד `0097_*`, `scripts/check-exemption-pin.ts`,
...
- **הנחות/פתוח/סיכון:** אין packaging conversion בלי יחס מאושר; snapshot אישור ממשיך לצרוך
  כמות ב־investigation עד הכרעת #130. הכרעת #131 נמסרה ב־`0097` כ־read-only, חלונות
...
- **קבצים/מיגרציות:** `0086`, `0089`, `interpret-document/core.ts`, בדיקות core/authorization,
...
- **מיגרציות:** צורך את `0089`–`0097`; אינו מסמיך את הדפדפן לכתיבה פיננסית ישירה.
- **בדיקות:** Vitest contracts, desktop/mobile fixtures, RTL/a11y/browser scenarios.
- **הנחות/פתוח/סיכון:** unknown=`—`, measured zero=`0`; UI ה־offboarding נצמד לפקודות שרת של `0097`.
...
- **קבצים/מיגרציות:** `outbox-worker/*`, shared Edge access guard, `0091`, `0094`, Supabase config.
...
- **הנחות/פתוח/סיכון:** Live Integration Proof הוא `DEFERRED` לפי הכרעת #128; אין יעד/credentials
  מומצאים. offboarding/export נמסר ב־`0097` וב־`tenant-export` ואינו הוכחת אינטגרציה לצד ג׳.
...
- **קבצים/מיגרציות:** review בלבד; ממצאיו הובילו לתיקונים ב־0092/0093/0094/0096/0097 וב־UI/Edge/tests/docs.
...
- **Migrations:** no migration owned by this addendum. The Edge export contract depends on the
  final `0097_tenant_offboarding_export.sql` RPCs and exact receipts. OCR evidence limits depend on
