...
function Assert-OcrPrerequisites([string]$Config) {
  $requiredFiles = @(
    ...
    "supabase\tests\p60_price_newest_effective_date.sql",
...
try {
  ...
  try {
    ...
    Invoke-SqlTest "supabase\tests\p60_price_newest_effective_date.sql" "Newest effective date wins the present: a backdated price records history and answers the as-of baseline for its own period, but never moves current_price -- on both the single write and the bulk import"
