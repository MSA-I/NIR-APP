...
try {
  ...
  try {
    ...
    Invoke-SqlTest "supabase\tests\p94_subscription_activation_email.sql" "The welcome email a paid activation owes is owed exactly once: the first verified activation records a debt addressed to the owner resolved from our own tables while a payload that names its own recipient changes nothing, a redelivered or resumed activation records no second row across a window longer than any provider idempotency header, a claim leases the row so a concurrent sender finds nothing, a replayed settlement is a no-op rather than a second send, an organization whose welcome already went out never becomes claimable again, an organization with no active owner records no debt and still receives the plan it paid for, and no tenant role can run either command"
