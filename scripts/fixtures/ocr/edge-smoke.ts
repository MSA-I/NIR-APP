...
if (workerToken.length < 32) throw new Error("OCR acceptance worker token is too short");
...
async function ok(label: string, promise: PromiseLike<QueryResult<unknown>>): Promise<void> {
  const result = await promise;
  assert.equal(result.error, null, `${label}: ${result.error?.message ?? "unknown error"}`);
}

async function must<T>(label: string, promise: PromiseLike<QueryResult<T>>): Promise<T> {
  const result = await promise;
  assert.equal(result.error, null, `${label}: ${result.error?.message ?? "unknown error"}`);
...
async function removeStorageFixture(): Promise<void> {
  ...
  assert.equal(removed.error, null, `remove Edge Storage fixture: ${removed.error?.message ?? "unknown error"}`);
  const listed = await admin.storage.from("documents").list(folder, { limit: 100, search: fileName });
  assert.equal(listed.error, null, `verify Edge Storage cleanup: ${listed.error?.message ?? "unknown error"}`);
  assert.equal((listed.data ?? []).some((item) => item.name === fileName), false,
...
async function main() {
  ...
  await ok("create organization", admin.from("organizations").insert({
    id: orgId,
    name: "OCR local Edge acceptance",
    status: "active",
  }));
  await ok("create supplier", admin.from("suppliers").insert({
    id: supplierId,
    org_id: orgId,
    name: "OCR acceptance supplier",
    status: "active",
  }));
  await ok("create product", admin.from("products").insert({
    id: productId,
    org_id: orgId,
  ...
  await ok("create supplier price", admin.from("supplier_products").insert({
  ...
  await ok("create owner profile", admin.from("profiles").insert({
    id: actorId,
    org_id: orgId,
  ...
  assert.equal(storagePath.startsWith(`${orgId}/supplier/${supplierId}/${documentId}/`), true);

  const pngBytes = Uint8Array.from(atob(
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
  ), (character) => character.charCodeAt(0));
  await ok("upload source document", user.storage.from("documents").upload(
    storagePath,
    pngBytes,
  ...
  assert.equal((wrongToken.payload.error as Record<string, unknown>)?.code, "invalid_worker_token");
  ...
  assert.equal("storage_path" in claimData, false, "worker response exposed storage_path");
  const signedDownload = new URL(String(claimData.download_url));
  const hostDownload = new URL(`${signedDownload.pathname}${signedDownload.search}`, apiUrl);
  ...
  try {
    interpreted = await interpretDocument(new Request(
  ...
  const interpretationResult = await interpreted.json() as Record<string, unknown>;
  assert.equal(interpreted.status, 200, JSON.stringify(interpretationResult));
  assert.equal(providerCalls, 1, "interpret handler did not call the focused provider mock exactly once");
  for (const forbidden of [storagePath, "download_url", "storage_path", "base64", "JVBERi0"]) {
    assert.equal(providerRequestText.includes(forbidden), false, `provider payload leaked ${forbidden}`);
  }
  ...
  const price = await must<{ current_price: number }>(
    ...
    admin.from("supplier_products").select("current_price").eq("id", supplierProductId).single(),
  );
  ...
  assert.equal(productCount.count, 1, "OCR confirmation created an unknown product");
  const job = await must<{ status: string }>(
    "read completed document job",
    admin.from("document_processing_jobs").select("status").eq("id", jobId).single(),
  );
...
if (runError) {
  if (cleanupError) console.error(`Edge Storage cleanup also failed: ${String(cleanupError)}`);
  throw runError;
}
