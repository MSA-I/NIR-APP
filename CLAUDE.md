...
- `npm run build` = `tsc --noEmit` + **תשעת** סקריפטי ה-`check:*` + `vitest run` (‏`npm run test`) + `vite build` — **השער האוטומטי היחיד.**
  ...
  רצופים גילו אותה שם.**
  ‏`npm run test` — ‏**426** בדיקות ב-**45** קבצים (נספר מפלט `vitest run`, ‏09.08.2026, אחרי חבילה 0
  של קמפיין המוכנות). **הערך שהיה רשום כאן, ‏400 ב-40, התיישן ב-4 קבצים לפני שנגעתי בקובץ** —
  ראה הערת הסטייה למטה.
