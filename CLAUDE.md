...
- `npm run build` = `tsc --noEmit` + **עשרת** סקריפטי ה-`check:*` + `vitest run` (‏`npm run test`) + `vite build` — **השער האוטומטי היחיד.**
  ...
  רצופים גילו אותה שם.**
  ‏`npm run test` — ‏**512** בדיקות ב-**60** קבצים (נספר מפלט `vitest run`, ‏10.08.2026, אחרי נרמול
  ...
  שהתיישנו. **הוא אינו סופר את מספר בדיקות ה-vitest** (‏512) — אי אפשר לקרוא אותו מטקסט בלי לממש
