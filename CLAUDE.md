...
- `npm run build` = `tsc --noEmit` + **תשעת** סקריפטי ה-`check:*` + `vitest run` (‏`npm run test`) + `vite build` — **השער האוטומטי היחיד.**
  ...
  רצופים גילו אותה שם.**
  ‏`npm run test` — ‏**422** בדיקות ב-**44** קבצים (נספר מפלט `vitest run`, ‏09.08.2026, אחרי חסימת ה-trial).
  ...
  שהתיישנו. **הוא אינו סופר את מספר בדיקות ה-vitest** (‏422) — אי אפשר לקרוא אותו מטקסט בלי לממש
...
- `npm run quality` — השער המלא (PowerShell + Docker): מאפס ובונה מחדש את `supplyflow-p0`, מריץ **28**
  ...
  **היסטוריית הסטייה, כי היא חזרה שש פעמים:** ‏13 → 20 → 26 → 27 → **28** לסוויטות; ‏22 → 25 → 29 → 30 → 33 →
  ...
  **מה `.github/workflows/quality-gate.yml` מריץ** — שלושה jobs **במקביל**, ולכן זמן הקיר הוא
  ...
  ‏`sql` (‏**28 סוויטות + preflight**) · ‏`browser` (‏**34 תרחישים** + fixtures + preview).
