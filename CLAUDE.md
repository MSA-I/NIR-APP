...
- `npm run quality` — השער המלא (PowerShell + Docker): מאפס ובונה מחדש את `supplyflow-p0`, מריץ **45**
  ...
  **איך נספר כל מספר כאן — כדי שהבא יספור ולא יעתיק:**
  - **45 סוויטות** = קריאות `Invoke-SqlTest` ב-`check-quality-gates.ps1` שהארגומנט הראשון שלהן הוא
    `supabase\tests\…`. בקובץ יש **48** מופעים של המחרוזת: אחד הוא הגדרת הפונקציה, ושניים טוענים
    fixtures (‏`supabase\demo\demo_seed.sql`, ‏`scripts\fixtures\ocr\browser-fixture.sql`).
    ב-`supabase/tests/` יש **46** קבצי `.sql`; ההפרש הוא `p1_preflight.sql`, שרץ דרך `Invoke-Preflight`.
  ...
  **היסטוריית הסטייה, כי היא חזרה אחת-עשרה פעמים:** ‏13 → 20 → 26 → 27 → 28 → 29 → 30 → 41 → 42 → 43 → 44 → **45** לסוויטות; ‏22 → 25 → 29 → 30 → 33 → 34 →
  **35** לתרחישים; ‏16 → 21 → **22** ל-`check:review`; ‏שבעה → שמונה → תשעה → **עשרה** לסקריפטי ה-`check:*`. בכל פעם הקובץ
  ...
  ארבעת הצעדים שאחריה הם קמפיין "מסמך ספק שהתקבל": ‏42 היא `p26_price_baseline.sql` (המחיר בתאריך
  ...
  לא מותאמת ל"הזמנה הפתוחה היחידה", תעודת משלוח כן), ו-**45 היא
  `p29_document_reconciliation_assessment.sql`** (‏`0108` — ארבעת המקורות: מה הוזמן, מה התקבל
  בקבלה **שהושלמה**, מה כתוב במסמך, ומה המחיר המוסכם **בתאריך המסמך**).
  ‏`check:counts` תפס את המספרים של כל אחד מארבעת הצעדים לפני הקומיט — ארבע פעמים ברצף.
  ...
  **מה `.github/workflows/quality-gate.yml` מריץ** — שלושה jobs **במקביל**, ולכן זמן הקיר הוא
  ...
  ‏`sql` (‏**45 סוויטות + preflight**) · ‏`browser` (‏**35 תרחישים** + fixtures + preview).
