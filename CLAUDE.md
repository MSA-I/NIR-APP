...
- `npm run quality` — השער המלא (PowerShell + Docker): מאפס ובונה מחדש את `supplyflow-p0`, מריץ **43**
  ...
  **איך נספר כל מספר כאן — כדי שהבא יספור ולא יעתיק:**
  - **43 סוויטות** = קריאות `Invoke-SqlTest` ב-`check-quality-gates.ps1` שהארגומנט הראשון שלהן הוא
    `supabase\tests\…`. בקובץ יש **46** מופעים של המחרוזת: אחד הוא הגדרת הפונקציה, ושניים טוענים
    fixtures (‏`supabase\demo\demo_seed.sql`, ‏`scripts\fixtures\ocr\browser-fixture.sql`).
    ב-`supabase/tests/` יש **44** קבצי `.sql`; ההפרש הוא `p1_preflight.sql`, שרץ דרך `Invoke-Preflight`.
  ...
  **היסטוריית הסטייה, כי היא חזרה תשע פעמים:** ‏13 → 20 → 26 → 27 → 28 → 29 → 30 → 41 → 42 → **43** לסוויטות; ‏22 → 25 → 29 → 30 → 33 → 34 →
  **35** לתרחישים; ‏16 → 21 → **22** ל-`check:review`; ‏שבעה → שמונה → תשעה → **עשרה** לסקריפטי ה-`check:*`. בכל פעם הקובץ
  ...
  שני הצעדים שאחריה הם קמפיין "מסמך ספק שהתקבל": ‏42 היא `p26_price_baseline.sql` (המחיר בתאריך
  המסמך), ו-**43 היא `p27_document_supplier_resolution.sql`** (סולם ראיות הספק, `0106`).
  ‏`check:counts` תפס את חמשת המספרים של הצעד הזה לפני הקומיט — שוב.
  ...
  **מה `.github/workflows/quality-gate.yml` מריץ** — שלושה jobs **במקביל**, ולכן זמן הקיר הוא
  ...
  ‏`sql` (‏**43 סוויטות + preflight**) · ‏`browser` (‏**35 תרחישים** + fixtures + preview).
