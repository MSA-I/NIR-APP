...
- `npm run build` = `tsc --noEmit` + **עשרת** סקריפטי ה-`check:*` + `vitest run` (‏`npm run test`) + `vite build` — **השער האוטומטי היחיד.**
  ...
  רצופים גילו אותה שם.**
  ‏`npm run test` — ‏**525** בדיקות ב-**62** קבצים (נספר מפלט `vitest run`, ‏10.08.2026 בערב, אחרי
  ‏`share.spec.ts`, גיליון הסינון ב-`dataTable.spec.tsx` והקליטה האחת ב-`documentsArchiveView.spec.tsx`;
  ...
  שהתיישנו. **הוא אינו סופר את מספר בדיקות ה-vitest** (‏525) — אי אפשר לקרוא אותו מטקסט בלי לממש
