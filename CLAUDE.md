...
- `npm run build` = `tsc --noEmit` + **אחד-עשר** סקריפטי ה-`check:*` + `vitest run` (‏`npm run test`) + `vite build` — **השער האוטומטי היחיד.**
  ...
  רצופים גילו אותה שם.**
  ‏`npm run test` — ‏**583** בדיקות ב-**67** קבצים (נספר מפלט `vitest run`, ‏11.08.2026 אחר הצהריים, אחרי
  ...
  שהתיישנו. **הוא אינו סופר את מספר בדיקות ה-vitest** (‏583) — אי אפשר לקרוא אותו מטקסט בלי לממש
