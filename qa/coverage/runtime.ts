...
/** Touch targets below the platform minimum are unusable on the receiving screen. */
export async function undersizedTouchTargets(page: Page, minimum = 40): Promise<string[]> {
  return page.evaluate((min: number) => {
    const results: string[] = [];
    for (const element of [...document.querySelectorAll('button,a[href],input,select,[role="button"]')]) {
      const rect = element.getBoundingClientRect();
...
export async function inspectSemantics(page: Page): Promise<{
  ...
  return page.evaluate(() => {
    ...
    const fieldsWithoutNames = [...document.querySelectorAll('input,select,textarea')]
      ...
      .map((field) => `${field.tagName.toLowerCase()}#${field.getAttribute('id') ?? 'anonymous'}`)
      .slice(0, 15);
    return {
