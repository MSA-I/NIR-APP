...
export function triage(aggregates: readonly RoleAggregate[]): TriagedObservation[] {
  ...
  const rules: TriageRule[] = [
    ...
    },
    {
      ...
      classify: (observation) => {
        const kitchen = observation.occurrences.filter((occurrence) => occurrence.role === 'kitchen');
        const others = observation.occurrences.length - kitchen.length;
        return {
          classification: 'CONFIRMED_DEFECT',
          rationale:
            `למטבח יש חוזה מובייל מפורש בסוויטה הקיימת (390x844, ללא גלישה, יעדי מגע 44px), ולכן ${kitchen.length} המופעים של המטבח הם פגם. ` +
            `${others} המופעים הנותרים בתפקידים אחרים אינם תחת החוזה הזה ודורשים הכרעת בעל מוצר בנפרד. ` +
            `המסלולים של המטבח: ${[...new Set(kitchen.map((occurrence) => occurrence.route))].join(', ')}.`,
        };
      },
    },
    {
