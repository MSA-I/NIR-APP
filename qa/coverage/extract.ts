...
/** Visible text of a JSX element: JsxText plus literal strings inside expressions. */
function jsxText(node: ts.Node): string {
  ...
  const walk = (child: ts.Node): void => {
    ...
    if (ts.isStringLiteral(child) || ts.isNoSubstitutionTemplateLiteral(child)) {
      const text = child.text.trim();
      // Class names and other machine strings are not visible text.
...
export function extractControls(file: string, repoRoot: string): ExtractedPageFacts {
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      const text = ts.isJsxElement(parent) ? jsxText(parent) : '';
      ...
      const accessibleName = [ariaLabel, label, searchLabel, text, placeholder, title].find(
