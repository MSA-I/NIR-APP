...
/** Tables a route's own code reads, as recorded in the manifest. */
function tablesFor(route: string): string[] {
  const entry = manifest.routes.find((candidate) => candidate.route === route);
  return [
    ...new Set(
      (entry?.knownActions ?? [])
        .flatMap((action) => action.backend)
        .filter((call) => call.startsWith('table:'))
        .map((call) => call.slice('table:'.length)),
    ),
  ];
}

/** Every table a role can legitimately reach through a screen it is allowed to open. */
function tablesReachableBy(role: QaRole): Set<string> {
  const reachable = new Set<string>();
  for (const row of routeMatrix.rows) {
    if (row.cells[role]?.verdict !== 'EXPECTED_ACCESS') continue;
    for (const table of tablesFor(row.route)) reachable.add(table);
  }
  return reachable;
}
...
class Recorder {
   ...
   *
   * The status matters: a screen the role cannot open has no states to miss, so filling those with
   * NOT_OBSERVED would count them against state coverage and make a correct denial look like a
   * testing gap. NOT_OBSERVED is reserved for a screen that rendered and a state that did not.
   */
  fillRemainingStates(
    applicable: readonly CoverageState[],
    status: StateCoverageStatus,
    rationale: string,
  ): void {
    const decided = new Set(this.states.map((record) => record.state));
    for (const state of applicable) {
      if (!decided.has(state)) this.state(state, status, rationale);
    }
  }
}
...
async function resolveConcreteRoute(page: Page, route: string): Promise<string | null> {
  ...
  const rowButton = page.locator('table tbody tr button, ul li button').first();
...
/**
 * Match one manifest control to one control on screen, and consume it.
 *
 * Matching is one-to-one: a runtime control that has been claimed leaves the pool, so two manifest
 * entries cannot both point at the same button and be counted twice. Containment is allowed in one
 * direction only — a short manifest label found inside a longer rendered name. The reverse would
 * let any long label swallow every short control on the page, which is exactly how an earlier
 * version reported 93% of the inventory as missing while the screen was fully rendered.
 */
function takeRuntimeMatch(
  pool: Map<string, RuntimeControl>,
  accessibleName: string | undefined,
): RuntimeControl | undefined {
  if (!accessibleName) return undefined;
  const target = normalizeName(accessibleName);
  if (target.length < 2) return undefined;

  for (const [key, control] of pool) {
    if (normalizeName(control.accessibleName) === target) {
      pool.delete(key);
      return control;
    }
  }
  if (target.length >= 4 && target.length <= 40) {
    for (const [key, control] of pool) {
      const candidate = normalizeName(control.accessibleName);
      if (candidate.length >= 4 && candidate.includes(target)) {
        pool.delete(key);
        return control;
      }
    }
...
/**
 * Wait for the screen to exist, not for the network to go quiet.
 *
 * `networkidle` can fall in the gap between the route chunk arriving and the data request starting,
 * so a snapshot taken on it measures the skeleton and reports an empty screen. The conditions below
 * are what "the route rendered" actually means: a heading owns the main region, and no skeleton is
 * left standing.
 */
...
for (const row of routeMatrix.rows) {
  ...
  test(`coverage ${row.route}`, async ({ page, qaRole, monitors }, testInfo) => {
    ...
    const tablesReachableByRole = tablesReachableBy(qaRole);
    ...
    if (verdict === 'NOT_APPLICABLE') {
      ...
      // PlatformGuard redirects from inside React, after the bundle mounts. Reading the path the
      // instant navigation resolves measures the URL before the guard has run at all, which would
      // report every role as having reached the console.
      await page
        .waitForURL((url) => url.pathname !== row.route, { timeout: 10_000 })
        .catch(() => undefined);
      await settle(page);
      ...
      recorder.state('permission_denied', rendered ? 'NOT_OBSERVED' : 'OBSERVED', rendered
        ...
        : `הופנה אל ${landed}.`);
      recorder.fillRemainingStates(applicableStates, 'NOT_APPLICABLE', 'מסלול מחוץ למודל תפקידי הדייר — מצבי המסך אינם ישימים לתפקיד זה.');
    ...
    if (verdict === 'EXPECTED_DENIAL' || verdict === 'UNKNOWN_REQUIRES_REVIEW') {
      ...
      //
      // Only tables this role cannot reach from ANY screen it is allowed to open are worth asking
      // about. `profiles` and `organizations` are read by the shell on every route, so a 200 on
      // them proves nothing — probing them would manufacture an authorization defect out of a
      // table the role is supposed to read. Exclusivity is what makes the answer mean something.
      const probeTargets = [...new Set(tablesFor(row.route))]
        .filter((table) => !tablesReachableByRole.has(table))
        .slice(0, 4);
      const probes: string[] = [];
      for (const table of probeTargets) {
        ...
        if (probe.status >= 200 && probe.status < 300 && (probe.rows ?? 0) > 0) {
          recorder.observe(
            ...
            `${qaRole} אינו רשאי להגיע ל-${row.route}, והטבלה ${table} אינה נקראת מאף מסך שהתפקיד כן רשאי לפתוח — ובכל זאת ${probe.endpoint} החזיר ${probe.status} עם ${probe.rows} שורות. הסתרה בצד לקוח אינה גבול הרשאה.`,
            'authorization',
            'high',
            evidence,
          );
      ...
      recorder.fillRemainingStates(applicableStates, 'NOT_APPLICABLE', 'התפקיד אינו רשאי לראות את המסך, ולכן מצבי המסך אינם ישימים עבורו.');
    ...
    if (!concrete) {
      recorder.fillRemainingStates(applicableStates, 'BLOCKED', 'לא נמצאה רשומת דוגמה להרכבת המסלול, ולכן המסך לא נבדק.');
    ...
    timings.navigation = Date.now() - started;
    await settle(page);
    timings.rendered = Date.now() - started;
    const landed = pathOf(page);
    ...
    if (landed !== concrete) {
      ...
      recorder.fillRemainingStates(applicableStates, 'BLOCKED', 'המסך לא רונדר, ולכן אין מצבים למדוד.');
      finish(testInfo, qaRole, row.route, {
    ...
    // One pool, drained as matches are claimed: whatever is left really was never in the manifest.
    const runtimePool = new Map(snapshot.controls.filter((control) => control.visible).map((control) => [control.key, control]));

    for (const { section, control } of manifestControls) {
      const match = takeRuntimeMatch(runtimePool, control.accessibleName);
      recorder.component({
        section,
    ...
    for (const control of runtimePool.values()) {
      recorder.component({
    ...
    for (const viewport of VIEWPORTS) {
      ...
      if (viewport.name === 'mobile') {
        const undersized = await undersizedTouchTargets(page);
        if (undersized.length) {
          recorder.observe(
            'יעדי מגע קטנים מ-44px',
            `${undersized.length} בקרות בתוך #main: ${undersized.slice(0, 8).join(', ')}.`,
            'visual',
