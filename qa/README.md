...
`qa:full` מבצע `setup → deterministic → agents (רק אחרי PASS דטרמיניסטי) → cleanup --keep-artifacts → report סופי`. ה־cleanup רץ גם כאשר שלב מאוחר נכשל, ותוצאתו נכללת בדוח הסופי שנשאר תחת `.qa-runs/<runId>/`. שלב הסוכנים נשאר אופציונלי לפי `QA_AGENT_ENABLED`.
...
ב־runners הישירים, exit code `0` מציין הצלחה או דילוג אופציונלי, `1` מציין כשל שבוצע ו־`2` מציין חסימה תפעולית. ה־JSON הוא מקור האמת: תמיד בודקים גם את `status`, `reason` והשלבים, ולא רק את exit code.

תרחיש `platform-admin` נשאר `BLOCKED` בכוונה. אין fixture מאושר לחשבון platform מקומי, ואסור לשדרג owner או משתמש tenant כדי לייצר PASS מלאכותי. ניתן לפתוח את התרחיש רק לאחר הגדרת fixture נפרד ומאושר ובדיקת בידוד מכל ששת תפקידי הדייר.
...
- כלי הדפדפן של הסוכן עדיין אינם מפיקים ראיית mutation מהימנה לאותו צעד; לכן פעולה עסקית משמעותית של סוכן AI נחסמת בבטחה עד שיוגדר חוזה ראיה כזה.
- לאחר העברה מוצלחת אין ב־UI של המשלם פעולה חוזרת על אותה בקשה, ולכן בדיקת retry דרך ה־UI נשארת `BLOCKED`.
- לשדה סיבת ייבוא הבנק אין שם נגיש שהבדיקה יכולה לבחור בבטחה; תרחיש ההתאמה נשאר `BLOCKED` בלי לנחש locator מבני.
