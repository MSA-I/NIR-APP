...
/**
 * Wait for the API to actually answer before walking anything.
 *
 * `qa:setup` finishes by restarting the database container, and PostgREST answers 503 for a short
 * window afterwards while it reconnects. A walk that starts inside that window records "the screen
 * rendered no heading" for the first few routes — a real observation about a broken stack, filed
 * as if it were a finding about the product. Measured against the wall clock rather than a retry
 * count, because a connection refused fails instantly and would burn a retry budget in seconds.
 */
async function waitForApiReady(supabaseUrl: string, anonKey: string, budgetMs = 120_000): Promise<boolean> {
  const deadline = Date.now() + budgetMs;
  while (Date.now() < deadline) {
    try {
      const response = await fetch(`${supabaseUrl}/rest/v1/`, { headers: { apikey: anonKey } });
      if (response.status < 500) return true;
    } catch {
      // Connection refused while the container comes back. Keep waiting until the clock says stop.
    }
...
export async function runCoverage(repoRoot = process.cwd()): Promise<{
  ...
  const apiReady = await waitForApiReady(state.environment.supabaseUrl, state.browserPublic!.supabaseAnonKey);
  if (!apiReady) {
    await releaseQaLock(handle);
    return {
      status: 'BLOCKED',
      reason: 'The local REST API did not stop returning 5xx within 120s. Walking now would record stack failures as product findings.',
      runId: state.runId,
    };
  }
