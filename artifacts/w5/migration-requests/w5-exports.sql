-- =====================================================================================
-- WAVE 5 MIGRATION REQUEST -- what leaves the system.
--
-- THIS FILE IS NOT A MIGRATION. It is the SQL Wave 5 needs, handed to the agent that owns
...
-- is a failure this repository has already paid for.
--
-- TWO CHANGES, INDEPENDENT OF EACH OTHER. They can ship in one migration or in two; nothing in
-- section 2 depends on section 1.
--
--   1. `private.product_purchase_summary` reports the COMMITTED cost it already reads and
--      discards, per order currency. ANCHORED PATCH of the live body.
--   2. `public.payment_reportable_amounts` -- a new read model for decision E. CREATE only;
--      no existing body is touched.
--
-- =====================================================================================
-- NO SECURITY DEFINER BODY IS REWRITTEN BY THIS FILE. Stated loudly because the question is
-- always asked and the answer here is unusually clean:
--
--   * `private.product_purchase_summary(uuid,date,date,uuid)` is a plain `LANGUAGE sql STABLE`
--     function. It is SECURITY INVOKER and it is not in `private.scope_definer_enforcements`
--     (verified: the only row matching `%product_purchase_summary%` is the public wrapper).
--   * The wrapper `public.get_product_purchase_summary(date,date,uuid)` IS pinned --
--     `filtered_read`, body hash `6aba764f7d94d4bbb9003f3e966fd02b` -- and this file does not
--     touch a byte of it. Its hash does not move, its scope proof still describes it exactly
--     ("aggregates only within auth_org after an explicit role check and returns per-product
--     totals rather than rows"), and the new figure is a per-product total like the others.
--   * `private.document_automation_authoritative_functions` holds no row for either.
--   * Section 2 creates a VIEW. `private.tenant_export_registry_violations()` inspects
--     `table_type = 'BASE TABLE'` only, so no registry row is required and none is added.
--
-- SO: NO PIN MOVES. If the migration author finds otherwise on the SHA they apply against, stop
-- and re-measure rather than adjusting a hash to make an assertion pass.
--
-- ---------------------------------------------------------------------------------------------
-- THE LIVE BODIES THIS FILE WAS WRITTEN AGAINST (printed with `pg_get_functiondef`, not
-- inferred, against `supabase_db_supplyflow-p0` on 2026-09-03)
--
--   private.product_purchase_summary(uuid,date,date,uuid)  -- last patched by 0221
--     `select poi.id, poi.product_id, poi.qty, poi.unit_price, poi.unit_snapshot,`
--     `       po.supplier_id, po.id as order_id`
--     -- `poi.unit_price` is SELECTED HERE AND NEVER READ AGAIN anywhere in the body. That one
--     -- fact is the whole of finding 2: the price the order committed to is already in the
--     -- grain and is thrown away.
--
--   public.payments RLS (`payments_select`)
--     `((org_id = auth_org()) AND (auth_role() = ANY (ARRAY['owner','accountant'])))`
--   public.invoices RLS (`invoices_select`)
--     `((org_id = auth_org()) AND ((auth_role() = ANY (ARRAY['owner','office']))
--       OR ((auth_role() = 'accountant') AND (review_status = 'approved'))))`
--   public.payment_allocations RLS (`pa_select`)
--     `((org_id = auth_org()) AND (auth_role() = ANY (ARRAY['owner','accountant'])))`
-- =====================================================================================
