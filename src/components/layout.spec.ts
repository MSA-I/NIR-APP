...
describe('סרגל הפעולות המהירות במובייל', () => {
  it('מחזיר פעולות מסך תפקידיות כשהצילום נמצא בדיוק באמצע', () => {
    ...
    expect(quickActionsFor('owner').map((item) => item.key)).toEqual(['dashboard', 'order', 'capture', 'documents', 'suppliers']);
    expect(quickActionsFor('office').map((item) => item.key)).toEqual(['dashboard', 'order', 'capture', 'documents', 'suppliers']);
