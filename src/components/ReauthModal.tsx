...
export interface ReauthModalProps {
   ...
   * final audit string: a caller that records a reason runs it through `reasonOr` (#299), so an
   ...
   * back through `onConfirm`. The box is OPTIONAL in the sense #299 fixed: it never disables the
 ...
 *
 * `reasonLabel` (added for #299) lets a flow that used to precede this dialog with a second,
...
export function ReauthModal({
  ...
  return (
    <Modal
      ...
      description={details ? `${details} ${t('reauthModal.description')}` : t('reauthModal.description')}
      busy={busy}
