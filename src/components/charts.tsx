...
export function TrendSparkline({ points, label }: { points: DashboardWeeklyPoint[]; label: string }) {
  ...
  const ariaLabel = `${label}: ${points.map((point) => `${point.week} ${point.count ? fmtMoneyExact(point.total) : t('charts.noRecords')}`).join(', ')}`;
        ...
        {slices.map((slice, index) => {
          ...
          return (
            <li key={slice.aggregate ? 'aggregate' : slice.name || index}>
              {href ? (
                <Link to={href} aria-label={hrefLabel?.(slice) ?? label} title={fmtMoneyExact(slice.total)}
