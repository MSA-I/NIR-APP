...
import {
  HEADLINE_QUOTA_KEY, PLAN_TRAY, PlanTicket, type PlanTicketFeature, PlanTicketSkeleton,
  RECOMMENDED_PLAN,
} from './PlanTicket';
              ...
              const quotaRows = option.contact_sales
                ...
                : planQuotas
                  .filter((row) => row.plan_key === option.plan_key && row.measured
                    && row.entitlement_key !== HEADLINE_QUOTA_KEY)
                  .map((row) => ({
                ...
                <PlanTicket
                  ...
                  figure={option.contact_sales ? t('orgSubscription.text_28')
                    : !option.paid ? t('orgSubscription.text_29')
                      : hasAmount ? fmtMoneyRounded(amount, displayCurrency) : t(PRICE_AT_UPGRADE_KEY)}
                  ...
                  /* The remaining quotas and the capability ladder, in the server's own wording:
                     `/pricing` prints the same string from the same function, and a card that
                     reworded it would be the second place a customer could read a different
                     sentence about one entitlement. The headline quota is NOT repeated here — it
                     is already the figure in the slot above. */
                  features={[...quotaRows, ...capabilityRows] satisfies PlanTicketFeature[]}
                />
              );
            })}
          </ul>
