...
describe('renderValue', () => {
  ...
  it('formats money through fmtMoneyExact, never on its own', () => {
    expect(renderValue(14, t, 'money', 'ILS')).toBe(fmtMoneyExact(14, 'ILS'));
    expect(renderValue(14, t, 'money', 'ILS')).toContain('14.00');
    // A numeric string out of jsonb is still money, not text.
    expect(renderValue('12.5', t, 'money', 'ILS')).toBe(fmtMoneyExact(12.5, 'ILS'));
    // Zero is a price, not a missing value.
    expect(renderValue(0, t, 'money', 'ILS')).toBe(fmtMoneyExact(0, 'ILS'));
    ...
    expect(renderValue(14, t, 'money', 'USD')).toBe(fmtMoneyExact(14, 'USD'));
    expect(renderValue(14, t, 'money', 'USD')).not.toBe(renderValue(14, t, 'money', 'ILS'));
  });
...
describe('fieldChanges', () => {
  ...
  it('pairs a price change as before and after, both formatted', () => {
    const changes = fieldChanges({ current_price: 12.5 }, { current_price: 14 }, t, 'ILS');
  ...
  it('calls a first-time value "not set" rather than leaving the side blank', () => {
    const changes = fieldChanges(null, { current_price: 14 }, t, 'ILS');
