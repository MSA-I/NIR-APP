  ...
  const balanceColumns: Column<InventoryBalance & { id: string }>[] = [
    ...
    },
    {
      ...
      render: (row) => row.cheapest_unit_price == null ? <span className="num">—</span> : (
        <span>
          <span className="block font-medium">{row.cheapest_supplier_name} · <span className="num">{fmtMoney(row.cheapest_unit_price)}</span></span>
          <span className="block text-xs text-ink-muted">
            {row.price_advantage == null ? 'מחיר פעיל יחיד' : `זול ב-${fmtMoney(row.price_advantage)} מהמחיר הבא`}
            {row.latest_purchase_unit_price == null ? '' : ` · רכישה אחרונה ${fmtMoney(row.latest_purchase_unit_price)}`}
          </span>
        </span>
      ),
    },
