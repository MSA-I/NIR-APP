...
describe('supplier metrics 90-day wording', () => {
   ...
   * screen renders that key, and the key still carries the exact phrase. Analytics has the same
   * split: its source pins the key while the dictionary pins the measured window.
   */
  it('uses the exact owner-approved phrase on supplier and analytics screens', () => {
    ...
    expect(analytics).toContain("t('analytics.priceChanges90')");
    expect(he.analytics.priceChanges90).toBe(PHRASE);
    expect(en.analytics.priceChanges90).toBe('Price changes (last 90 days)');
    for (const wrongWindow of ['שינויי מחיר (180 יום)', 'שינויי מחיר (30 יום)']) {
      ...
      expect(he.analytics.priceChanges90).not.toContain(wrongWindow);
