          ...
          <div className="grid gap-3 lg:grid-cols-3">
            ...
            <div className="card card-pad">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
                ...
                <Metric label="עלות" value={fmtMoney(metrics.usage.cost)} />
              </dl>
...
function CalibrationQueueSection({ query, onReview }: {
  ...
  const columns: Column<CalibrationQueueRow & { id: string }>[] = [
    ...
    },
    {
      ...
      render: (row) => <span className="num">{fmtMoney(row.current_unit_price)} ← {fmtMoney(row.proposed_unit_price)}</span>,
    },
    {
      ...
      {row && <form className="space-y-5" onSubmit={(event) => void submit(event)}>
        <div className="card card-pad">
          <div className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Metric label="מחיר חזוי" value={fmtMoney(row.proposed_unit_price)} />
          ...
          {attempts.map((attempt, index) => (
            <article key={attempt.job_id} className="rounded-xl border border-line-soft p-4">
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                ...
                <Metric label="עלות מדווחת" value={fmtMoney(attempt.usage_cost)} />
              </dl>
