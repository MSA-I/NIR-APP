    ...
    <div className="space-y-4">
        ...
        <section className="note-info flex-wrap items-center" aria-label={t('supplierSplit.aria_label')}>
          <div className="min-w-0 flex-1">
            ...
            <span className="mt-0.5 block text-xs">
              <ConsolidationCost savings={split.savings.savings} /> {t('supplierSplit.fmtMoneyExact')} <span className="num font-semibold">{fmtMoneyExact(split.savings.singleSupplierTotal)}</span>
            </span>
          </div>
      ...
      <section aria-labelledby="selected-products-title" className="border-y border-line-strong bg-surface">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line-soft px-3 py-3 sm:px-4">
          ...
          <span className="text-sm text-ink-muted">{t('supplierSplit.fmtMoneyExact_2')} <b className="num text-ink">{fmtMoneyExact(split.total)}</b></span>
        </div>
            ...
            <div key={line.productId} className={`flex flex-wrap items-center gap-2 px-2 py-3 ${line.status === 'pin_below_min_qty' ? 'bg-await-wash text-await-fg' : ''}`}>
              ...
              {line.status === 'pin_below_min_qty' && requiredQty != null && (
                <button type="button" className="btn-secondary btn-sm" onClick={() => onQty(line.productId, requiredQty)}>
                  {t('supplierSplit.raiseTo')}<span className="num">{requiredQty}</span>{addedCost != null ? <> · <span className="num">+{fmtMoneyExact(addedCost)}</span></> : null}
                </button>
              )}
    ...
    <section aria-labelledby="supplier-comparison-title" className="border-y border-line-strong bg-surface">
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-line-soft px-3 py-3 sm:px-4">
        ...
        <div className="text-start sm:text-end">
          ...
          {extra != null && (
            <span className="mt-0.5 block text-xs text-await-fg">{t('supplierSplit.fmtMoneyExact_3')} <span className="num">{fmtMoneyExact(extra)}</span> {t('supplierSplit.fmtMoneyExact_4')}</span>
          )}
        </div>
      </div>
      <div className="divide-y divide-line-soft">
...
function ConsolidationCost({ savings }: { savings: number | null }) {
  ...
  return savings > 0
    ? <>{t('supplierSplit.fmtMoneyExact_5')} <span className="num font-semibold">{fmtMoneyExact(savings)}</span></>
    : <>{t('supplierSplit.fmtMoneyExact_6')} <span className="num font-semibold">{fmtMoneyExact(Math.abs(savings))}</span></>;
}
...
function LineComparisonNote({ comparison }: { comparison: LineComparison }) {
  ...
  if (comparison.status === 'saved') {
    return <p className="mt-0.5 text-xs text-done-fg">{t('supplierSplit.fmtMoneyExact_7')} <span className="num">{fmtMoneyExact(comparison.savedVsNext)}</span> {t('supplierSplit.fmtMoneyExact_8')}</p>;
  }
  if (comparison.status === 'overpaying') {
    return <p className="mt-0.5 text-xs text-await-fg">{t('supplierSplit.fmtMoneyExact_9')} <span className="num">{fmtMoneyExact(comparison.extraVsCheapest)}</span> {t('supplierSplit.fmtMoneyExact_10')}</p>;
  }
