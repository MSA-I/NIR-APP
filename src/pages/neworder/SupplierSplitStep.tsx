          ...
          {cart.map((item) => {
            ...
            return (
              <div key={item.productId} className="grid items-center gap-2 px-3 py-3 sm:grid-cols-[minmax(0,1fr)_auto_minmax(9rem,auto)_auto_2.75rem] sm:gap-4 sm:px-4">
                ...
                <button type="button" className="grid size-11 place-items-center rounded-lg text-ink-faint hover:bg-surface-hover hover:text-alert-fg" onClick={() => onRemove(item.productId)} aria-label={`הסרת ${item.product.name}`}><Trash2 size={15} /></button>
              </div>
            );
          })}
        </div>
          ...
          return (
            <div key={item.productId} className="px-3 py-3 text-sm sm:px-4">
              <div className="mb-2 flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0"><div className="font-medium text-ink-body"><bdi>{item.product.name}</bdi></div><LineComparisonNote comparison={comparison} /></div>
