    ...
    <div className="space-y-4">
        ...
        <section className="note-await block" aria-labelledby="minimum-summary-title">
          ...
          <ul className="mt-2 space-y-1 text-xs">
            {breaches.map((group) => <li key={group.supplier.id}><bdi>{group.supplier.name}</bdi>{' '}
              {t('summaryStep.shortfallPrefix')} <span className="num font-semibold">{fmtMoneyExact(group.shortfall)}</span></li>)}
          </ul>
        </section>
      )}
      ...
      <section aria-labelledby="summary-title" className="border-y border-line-strong bg-surface">
        ...
        <div className="grid grid-cols-2 gap-px bg-line-soft sm:grid-cols-4">
          ...
          <SummaryMetric label={t('summaryStep.label_3')} value={fmtMoneyExact(savings.splitTotal)} />
          <SummaryMetric label={savings.savings != null && savings.savings < 0
            ? t('summaryStep.extraCostMetric')
            : t('summaryStep.savingsMetric')} value={savings.savings == null ? '—' : `${fmtMoneyExact(Math.abs(savings.savings))} (${Math.abs(savings.savingsPercent ?? 0).toFixed(1)}%)`} tone={savings.savings != null && savings.savings >= 0 ? 'done' : 'await'} />
        </div>
        {savings.singleSupplierTotal == null ? <p className="border-t border-line-soft px-3 py-3 text-sm text-ink-muted sm:px-4">{t('summaryStep.text_2')}</p>
          : <p className="border-t border-line-soft px-3 py-3 text-sm text-ink-muted sm:px-4">{t('summaryStep.singleSupplierAlternative')}{' '}
            <bdi>{singleSupplierName ?? t('summaryStep.genericSupplier')}</bdi> · <span className="num font-semibold text-ink">{fmtMoneyExact(savings.singleSupplierTotal)}</span></p>}
      </section>
        ...
        <div className="divide-y divide-line-strong">
          {split.groups.map((group) => (
            <div key={group.supplier.id}>
              <div className="flex flex-wrap items-center justify-between gap-2 bg-surface-sunken px-3 py-2.5 sm:px-4"><strong><bdi>{group.supplier.name}</bdi></strong><span className="num font-semibold">{fmtMoneyExact(group.subtotal)}</span></div>
              <div className="divide-y divide-line-soft">
                {group.lines.map((line) => <div key={line.productId} className="grid gap-1 px-3 py-2.5 text-sm sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:gap-5 sm:px-4"><span className="font-medium text-ink-body"><bdi>{products.get(line.productId)?.name ?? t('summaryStep.map')}</bdi></span><span className="text-ink-muted">{t('summaryStep.map_2')} <span className="num text-ink">{line.qty}</span></span><strong className="num sm:text-end">{fmtMoneyExact(line.lineTotal)}</strong></div>)}
              </div>
            </div>
          ))}
        </div>
