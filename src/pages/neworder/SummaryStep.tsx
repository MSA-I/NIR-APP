    ...
    <div className="space-y-4">
      {breaches.length > 0 && (
        <section className="note-await block" aria-labelledby="minimum-summary-title">
          ...
          <ul className="mt-2 space-y-1 text-xs">
            {breaches.map((group) => <li key={group.supplier.id}>{group.supplier.name} · חסרים <span className="num font-semibold">{fmtMoneyExact(group.shortfall, group.currency)}</span></li>)}
          </ul>
        </section>
      )}
      ...
      <section aria-labelledby="summary-title" className="border-y border-line-strong bg-surface">
        ...
        <div className="grid grid-cols-2 gap-px bg-line-soft sm:grid-cols-4">
          ...
          <SummaryMetric label="עלות לאחר חלוקה" value={fmtMoneyExact(savings.splitTotal, savings.currency)} />
          <SummaryMetric label={savings.savings != null && savings.savings < 0 ? 'תוספת עלות לעומת ספק יחיד' : 'חיסכון לעומת ספק יחיד'} value={savings.savings == null ? '—' : `${fmtMoneyExact(Math.abs(savings.savings), savings.currency)} (${Math.abs(savings.savingsPercent ?? 0).toFixed(1)}%)`} tone={savings.savings != null && savings.savings >= 0 ? 'done' : 'await'} />
        </div>
        {savings.singleSupplierTotal == null ? <p className="border-t border-line-soft px-3 py-3 text-sm text-ink-muted sm:px-4">אין ספק יחיד שמציע את כל מוצרי הסל, ולכן לא מוצגת טענת חיסכון.</p>
          : <p className="border-t border-line-soft px-3 py-3 text-sm text-ink-muted sm:px-4">חלופת ספק יחיד: {singleSupplierName ?? 'ספק'} · <span className="num font-semibold text-ink">{fmtMoneyExact(savings.singleSupplierTotal, savings.currency)}</span></p>}
      </section>
        ...
        <div className="divide-y divide-line-strong">
          {split.groups.map((group) => (
            <div key={group.supplier.id}>
              <div className="flex flex-wrap items-center justify-between gap-2 bg-surface-sunken px-3 py-2.5 sm:px-4"><strong>{group.supplier.name}</strong><span className="num font-semibold">{fmtMoneyExact(group.subtotal, group.currency)}</span></div>
              <div className="divide-y divide-line-soft">
                {group.lines.map((line) => <div key={line.productId} className="grid gap-1 px-3 py-2.5 text-sm sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:gap-5 sm:px-4"><span className="font-medium text-ink-body">{products.get(line.productId)?.name ?? 'מוצר'}</span><span className="text-ink-muted">כמות <span className="num text-ink">{line.qty}</span></span><strong className="num sm:text-end">{fmtMoneyExact(line.lineTotal, line.currency)}</strong></div>)}
              </div>
            </div>
          ))}
        </div>
