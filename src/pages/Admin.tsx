  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <input className="input" readOnly value={value} dir="ltr" />
        <button className="btn-secondary p-2!" aria-label={`העתקת ${label}`}
          onClick={() => { void navigator.clipboard.writeText(value); onCopy(); }}>
          <Copy size={16} />
        </button>
      </div>
    </div>
    ...
    <Modal open={open} onClose={close} title="הקמת ארגון חדש" wide>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="label">אימייל בעל העסק</label>
            <input className="input" type="email" dir="ltr" value={form.ownerEmail} onChange={(e) => set('ownerEmail', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="label">סיסמה ראשונית (לפחות 10 תווים)</label>
            <div className="flex items-center gap-2">
              <input className="input" dir="ltr" value={form.password} onChange={(e) => set('password', e.target.value)} />
              <button className="btn-secondary whitespace-nowrap" onClick={() => set('password', generatePassword())}>הגרלה מחדש</button>
            </div>
          </div>
          <div>
            <label className="label">שיעור מע״מ (%)</label>
            <input className="input num" type="number" step="0.5" value={form.vatRate} onChange={(e) => set('vatRate', e.target.value)} />
          </div>
          <div>
            <label className="label">סיום תקופת ניסיון (אופציונלי)</label>
            <input className="input" type="date" min={todayISO()} value={form.trialEndsAt} onChange={(e) => set('trialEndsAt', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className="label">קטגוריות בסיס (מופרדות בפסיק — ריק יוצר «כללי» בלבד)</label>
            <input className="input" value={form.categories} onChange={(e) => set('categories', e.target.value)} />
          </div>
        </div>
