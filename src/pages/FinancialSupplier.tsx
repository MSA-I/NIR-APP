...
export default function FinancialSupplier() {
  ...
  const { data, loading, error } = useQuery(async () => {
    ...
    const activity = [
      ...invoices.map((row) => ({ date: row.invoice_date, label: t('financialSupplier.activityInvoice', { number: row.invoice_number }), amount: row.total_amount })),
      ...payments.map((row) => ({ date: row.paid_date, label: t('financialSupplier.activityPayment', { number: row.number }), amount: -row.amount })),
      ...credits.filter((row) => ['offset', 'closed'].includes(row.status))
        .map((row) => ({ date: row.created_at, label: t('financialSupplier.activityCredit', { number: row.number }), amount: -row.amount })),
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Card><div className="text-sm text-ink-muted">{t('financialSupplier.fmtMoneyRounded')}</div><div className="mt-1 kpi-value num">{fmtMoneyRounded(data.openBalance)}</div></Card>
        <Card><div className="text-sm text-ink-muted">{t('financialSupplier.fmtMoneyRounded_2')}</div><div className="mt-1 kpi-value num">{fmtMoneyRounded(data.dueExposure)}</div></Card>
          ...
          <div className="mt-3 divide-y divide-line">
            {data.invoices.map((invoice) => {
              ...
              return <Link key={invoice.id} to={`/invoices/${invoice.id}`} className="flex min-h-12 items-center justify-between gap-3 py-2">
                ...
                <span className="text-end"><StatusBadge meta={INVOICE_PAYMENT_STATUS[invoice.payment_status]} /><span className="ms-2 num">{fmtMoneyRounded(balance?.balance ?? null)}</span></span>
              </Link>;
            })}
          </div>
        )}
        ...
        <Card as="section" aria-labelledby="finance-payments">
          ...
          {!data.payments.length ? <EmptyState title={t('financialSupplier.title_2')} /> : <div className="mt-3 divide-y divide-line">
            {data.payments.map((payment) => <div key={payment.id} className="flex min-h-12 items-center justify-between gap-3 py-2 text-sm">
              ...
              <span className="text-end num">{fmtMoneyRounded(payment.amount)}<span className="block text-xs text-ink-muted">{t('financialSupplier.allocatedLabel')} {fmtMoneyRounded(data.allocatedByPayment.get(payment.id) ?? 0)}</span></span>
            </div>)}
          </div>}
        </Card>
