      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {canWrite && (
          <div className="flex flex-wrap gap-2">
            <button className="btn-secondary" onClick={() => setImportOpen(true)}><Upload size={15} /> ייבוא מ־Excel/CSV</button>
...
function PriceDocumentModal({ orgId, onClose }: { orgId: string; onClose: () => void }) {
  ...
  const { data: suppliers, loading, error } = useQuery(async () =>
    unwrap(await supabase.from('suppliers').select('id, name').is('deleted_at', null).order('name')) as Pick<Supplier, 'id' | 'name'>[]);

  async function submit() {
    if (!supplierId) { toast('יש לבחור ספק למסמך המחירון', 'error'); return; }
  ...
  return (
    <Modal open onClose={onClose} title="העלאת מחירון כמסמך" busy={busy} statusMessage={busy ? 'שומר ומכניס את המסמך לעיבוד' : undefined}>
      {pendingReservation ? (
        ...
        <div className="space-y-4">
          <Note tone="info">המקור נשמר כמסמך ספק ומועבר למסך בדיקה. מחיר ייכתב רק לאחר אישור; שורה לא מזוהה ניתנת לשיוך למוצר קיים בלבד.</Note>
          {error ? <ErrorNote message={error} /> : (
            <label className="block">
              <span className="label">ספק *</span>
              <select className="input" value={supplierId} disabled={loading || busy} onChange={(event) => setSupplierId(event.target.value)}>
                <option value="">{loading ? 'טוען ספקים…' : 'בחירת ספק'}</option>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="btn-primary" disabled={busy || !!error || loading} onClick={() => void submit()}>העלאה והמשך לבדיקה</button>
          </div>
        </div>
      )}
    </Modal>
