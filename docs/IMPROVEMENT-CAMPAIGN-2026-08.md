...
**מקור אמת:** קוד ומיגרציות `0093`–`0103`, לאחר מיזוג `origin/main` שכבר כלל את `0086`–`0089`.
...
- `trial_ends_at` היה מידע לפני `0092`; לא היה read-only אוטומטי לאחר grace.
...
הכרעות הבעלים המלאות נמצאות ב־`OPEN-DECISIONS.md` #133–#138 (מוספרו מחדש במיזוג: ‏`main` כבר
החזיקה ‏#124–#126 משלה):
...
| מיגרציה | שינוי מרכזי |
|---|---|
| `0093` | reprocess מנומק, matching מחירון לפי מזהים חזקים, rollback אוטומציה |
| `0094` | trigger שחוסם פעילות מסחרית חדשה עם ספק inactive |
| `0095` | SQL lexer ל־scope marker ורשם body-hash/proof |
| `0096` | shadow/calibration ledgers, corpus queue, drift ו־document operations read models |
| `0097` | `read_financial_supplier` וגבול role/tenant פיננסי |
| `0098` | מיתוג ארגוני ו־Storage policy לוגו דיירי |
| `0099` | invoice line evidence/matches/overrides, assessment ו־approval guard |
| `0100` | `management_dashboard_snapshot(date)` |
| ~~`0092`~~ | Trial ‏30+7, access mode, write guards ו־Storage write latch — **אינה חלק מהקמפיין הזה יותר.** נכתבה כאן כ-`0108`, אומצה ל-`main` בהכרעת בעלים ב-10.08.2026 כ-`0092` ו**כבר הוחלה בייצור**. השורה נשארת כדי שהקורא לא יחפש אותה |
| `0101` | supplier portal projection והרחבת transition PO לספק שלו בלבד |
| `0102` | `inventory_intelligence` read model |
| `0103` | offboarding lifecycle, durable export parts/manifest, revocable delivery, egress leases ו־worker fencing |
...
- `0095` דוחה marker ב־comment/literal ומחייב hash של גוף פונקציה שנבדק.
...
| DB reset/migrations | `PASS` | reset נקי ומסלול upgrade עד `0103`, כולל reset בידוד סופי |
...
2. לוודא ש־`0086`–`0092` של `origin/main` כבר הוחלו — ‏**כולל `0092`, שהיא ה-`0108` של הקמפיין
   ונפרסה לייצור ב-10.08.2026 לפני כל השאר** — ואז להחיל בסדר עולה בלבד: `0093` → `0094` →
   `0095` → `0096` → `0097` → `0098` → `0099` → `0100` → `0101` → `0102` → `0103`.
   ‏**היפוך הסדר הזה אינו קוסמטי:** ‏`0092` מצמידה את בריח הקריאה-בלבד בלולאה חד-פעמית על
   `information_schema`, ולכן כל טבלת `org_id` ציבורית שנוצרת אחריה חייבת `create trigger
   zz_organization_write_guard` מפורש. ‏`0096` (חמש טבלאות) ו-`0099` (שש) קיבלו אותו במיזוג;
   ‏`p22_trial_read_only` אוכפת את הכלל כטענה אוניברסלית.
...
ההחלטות החדשות #133–#138 סגורות. #140 הוכרעה ומומשה בחוזה offboarding/export; #139 נשארת פתוחה
