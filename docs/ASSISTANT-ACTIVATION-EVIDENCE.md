...
מצב נכון ל-**30.08.2026**. **חמש מתוך חמש `VERIFIED`.** ארבע נשלפו מהמקורות הרשמיים של OpenAI
ב-24.08; החמישית, `dpa`, נסגרה ב-30.08 על **הסכם חתום** ולא על היתר.

> **‏30.08.2026 — ה-DPA נחתם, ולכן ההיתר נמחק. שימו לב מה בדיוק נסגר ומה לא.**
> ‏`InPlace` ו-`OpenAI OpCo, LLC` חתמו על ‏`OpenAI Data Processing Addendum v.010126` ב-30.08.2026
> (‏DocuSign, מזהה מעטפה `29AC0D70-…`; העותק החתום נשמר **מחוץ לריפו** ב-`NIR-APP-DOCS` ואינו
> נכנס ל-Git). שורת `dpa` בטבלה שלמטה עברה ל-`VERIFIED` על ראיה אמיתית, ו-
> `AI_ASSISTANT_PRELAUNCH_EXCEPTION` **הוסר מסודות הייצור באותו יום** — לא פג, נמחק. השער עומד
> עכשיו על חמש שורות ראיה בלבד, וזה נמדד: ראה יומן האימות (§5, ‏30.08.2026).
>
> **מה שהחתימה אינה משנה, ואסור שתיקרא כאילו כן.** ‏ה-DPA **אינו** הסכם אפס-שימור, **אינו**
> מוסיף התחייבות שימור של 30 יום (סעיף 3.3 בו מטיל את הגדרות השימור על הלקוח — ראה ממצא 4 למטה)
> ו**אינו** מגדיר אזור נתונים. שלוש השורות האחרות אומרות בדיוק מה שאמרו אתמול, ו-`/privacy`
> ו-`/terms` לא השתנו במילה — ולכן `TERMS_VERSION` **לא הועלה**. חוזה שנחתם אינו רישיון להבטיח
> יותר; הוא סוגר שורה אחת בדיוק.
>
> **הרישום ההיסטורי למטה נשאר כלשונו** — הוא מתעד איך המערכת רצה בין 25.08 ל-30.08, וזה בדיוק
> מה שביקורת צריכה לקרוא. ריצה מאותו חלון נשארת „פעלה תחת היתר טרום-השקה", לעולם לא „היה חוזה".

> **‏25.08.2026 (היסטורי — ההיתר הוסר ב-30.08) — קיים היתר טרום-השקה (`#271`), והטבלה הזו לא זזה. זה לא פיקוח, זו הנקודה.**
> ההיתר אינו ראיה: הוא **אינו** הופך את `dpa` ל-`VERIFIED`, אינו מוסיף מקור, ואינו משנה מילה
> ב-`/privacy`. השורה נשארת `MISSING` מפני שזו **האמת** — אין חוזה — ודף הפרטיות ממשיך לומר
> בדיוק את מה שנכון. מה שההיתר כן עושה הוא לאפשר הרצה **מוגבלת**: ארגון אחד, עד תאריך אחד,
> ורק על השורה הזו. כל שאר השורות עומדות על ראייתן.
>
> **ולכן, כשהשער נפתח עכשיו, הוא נפתח על משהו אחר ממה שנראה.** ריצה תחת ההיתר נרשמת כ„פעל תחת
> היתר טרום-השקה", לעולם לא כ„היה חוזה". מסמך זה אינו מייצר הרשאה, וגם הרשאה אינה מייצרת שורה בו.

> **מה השתנה ב-25.08.2026 (היסטורי), וזו הבחנה שמשנה את המשמעות של השורה החסרה.** עד אז נרשם כאן שהבעלים
...
| `dpa` | `dpa_executed_openai_opco_llc_v010126` — ‏**DPA חתום בין `InPlace` לבין `OpenAI OpCo, LLC`** (הישות הנכונה: הלקוח אינו באזור הכלכלי האירופי ולא בשווייץ). ‏**חוזה:** ‏`OpenAI-DPA-v.010126`, ‏DocuSign מזהה מעטפה `29AC0D70-…` — **המזהה המלא נמצא בסוד `AI_ASSISTANT_GOVERNANCE_DPA` ובעותק החתום מחוץ לריפו, ואינו נכתב לכאן: הריפו ציבורי** | `https://openai.com/policies/data-processing-addendum/` (v.010126, ‏Effective: January 1, 2026 — **הנוסח שנחתם**) | 2026-08-30 | ‏`Nir Barmuha` (בעלים, החותם מטעם `InPlace`; מטעם OpenAI: ‏`Dan Cook, Authorized Signer`) | `VERIFIED` |
...
- **`dpa`** — עד 30.08.2026 הציטוט כאן היה על **היכולת** לחתום: „Yes, we are able to execute a Data
  Processing Addendum (DPA) with customers … Please complete our DPA form to execute a DPA with
  OpenAI." מ-30.08.2026 השורה עומדת על החוזה **החתום** עצמו, ‏`v.010126` סעיף 1.1: „OpenAI acts as
  a Data Processor on the Customer's behalf, and this DPA governs such Processing." המסמך אינו
  בריפו; הציטוט הוא מהעותק החתום שנקרא ב-30.08.2026.
...
### איך `dpa` נסגרה (30.08.2026), ומה החתימה לא סגרה

התהליך היה מתועד ואומת מ-24.08: ‏OpenAI מציעה DPA ל-API, ‏חתימה click-through, ולקוח באזור
הכלכלי האירופי מתקשר מול `OpenAI Ireland Ltd.` ואחרת מול `OpenAI OpCo, LLC`. **מה שחסר היה לא
התהליך אלא העובדה** — האם **החשבון הזה** חתם. זו עובדה על החשבון, אין עמוד בעולם שעונה עליה,
והיא יכלה להירשם רק על ידי מי שיש לו גישה לחשבון.

**היא נרשמה ב-30.08.2026, והפעם על מסמך ולא על אמירה.** ההסכם החתום —
`OpenAI Data Processing Addendum v.010126` — נמסר על ידי הבעלים ונקרא במלואו לפני שנכתבה כאן
שורה. מה שנקרא בו, ומה שמצדיק את `VERIFIED`:

| מה נבדק במסמך | מה נמצא |
|---|---|
| ‏שני הצדדים חתומים | כן — שתי חתימות, שתיהן בתאריך **30.08.2026**; ה-`Effective Date` בחוזה מוגדר „date last signed by a Party", כלומר אותו יום |
| הישות המתקשרת | `OpenAI OpCo, LLC` — **הנכונה**: ‏`OpenAI Ireland Ltd.` חלה רק על לקוח באזור הכלכלי האירופי או בשווייץ, וכתובת הלקוח בחוזה היא בישראל |
| מי חתם מטעמנו | ‏`Nir Barmuha`, ‏`Founder` |
| מי חתם מטעם הספק | ‏`Dan Cook`, ‏`Authorized Signer` |
| גרסת הנוסח | ‏`v.010126` בכותרת כל עמוד — אותה גרסה שהעמוד הרשמי מפרסם כ-Effective January 1, 2026 |
| מזהה החתימה | ‏DocuSign Envelope ID, מוטבע בכל עמוד. **המזהה המלא אינו נכתב לריפו הציבורי** — הוא נמצא בסוד `AI_ASSISTANT_GOVERNANCE_DPA` (שדה `contract`) ובעותק החתום |
| ‏`OpenAI` כמעבד | סעיף 1.1: ‏„OpenAI acts as a Data Processor on the Customer's behalf" |

**ומה שהמסמך במפורש אינו נותן — ולכן אף שורה אחרת לא זזה:**

1. **אין בו סעיף שימור של 30 יום.** סעיף 3.3 (`Configurations`) מטיל את הגדרות השימור והמחיקה
   על **הלקוח**. מגבלת 30 היום חיה בתיעוד וב-FAQ של הספק — מסמכים שהוא רשאי לשנות חד-צדדית —
   ולא בחוזה. ‏**אין לצטט את ה-DPA כמקור למגבלת 30 היום** (זהו ממצא 4 למעלה, והוא אומת שוב מול
   הנוסח החתום עצמו).
2. **אין בו אפס-שימור.** ‏`zero_retention` דורש אישור מוקדם והסכם נפרד; ה-DPA אינו זה. שורת
   `retention` נשארת `retention_up_to_30_days_default`, ו-`#179` עומד בלי שינוי.
3. **אין בו אזור נתונים.** שורת `data_region` נשארת `no_data_residency_configured_unrestricted_processing`.
4. **אין בו רישום חברה.** המסמך מוכיח שנחתם חוזה; הוא **אינו** מוכיח שנרשמה חברה. השורה שנסגרה
   כאן היא `dpa` בלבד, ומי שמחפש את שאלת הישות המשפטית ימצא אותה ב-`DEBT-REGISTER §63`.

**ולכן `/privacy` ו-`/terms` לא השתנו במילה, ו-`TERMS_VERSION` לא הועלה.** דף הפרטיות מעולם לא
טען „אין לנו DPA" — הוא תיאר שימור, עיון אנושי ומיקום עיבוד, וכל השלושה נכונים היום בדיוק כפי
שהיו אתמול. חוזה חדש שמשנה טקסט שלא השתנה הוא הצהרה חדשה ללקוח, וזה מה שהגרסה קיימת כדי למנוע.
...
**‏`AI_ASSISTANT_PRELAUNCH_EXCEPTION` אינו מוגדר בייצור מ-30.08.2026.** משטח ההיתר עדיין קיים
בקוד (`governance.ts`), אך אין לו ערך — וכל עוד אין, השער עומד על חמש השורות בלבד. **החזרתו היא
הכרעה, לא תיקון**, והיא תדרוש שורה חדשה ביומן האימות שתסביר איזו ראיה נסוגה.

**איך מאמתים ערך בלי לחשוף אותו.** ‏`GET /v1/projects/{ref}/secrets` מחזיר `sha256` של הערך ולא
את הערך. ‏`sha256` של המחרוזת שהתכוונו לכתוב, מול מה שה-API מחזיר — התאמה היא הוכחה שהבייטים
הנכונים בייצור, והיא אינה מדליפה דבר. זו הדרך שבה אומתה שורת `dpa` ב-30.08.2026, ואין צורך
בשום דרך אחרת.
...
| 30.08.2026 | `dpa` | קריאת ההסכם החתום שנמסר על ידי הבעלים | ‏**נחתם.** ‏`OpenAI Data Processing Addendum v.010126`, ‏`InPlace` ↔ `OpenAI OpCo, LLC`, שתי חתימות ב-30.08.2026, ‏DocuSign Envelope ID מוטבע בכל עמוד. אומת גם מה **שאין** בו: אין סעיף שימור 30 יום (סעיף 3.3 מטיל את ההגדרה על הלקוח), אין אפס-שימור, אין אזור נתונים — ולכן אף שורה אחרת לא זזה | בעלים (מסר את המסמך) · סוכן (קרא את חמשת העמודים) |
| 30.08.2026 | `dpa` | ‏`AI_ASSISTANT_GOVERNANCE_DPA` הוגדר בייצור | הערך נכתב בפורמט של §3 ואומת **בלי להיחשף**: ה-Management API שומר digest, ו-`sha256` של המחרוזת המיועדת זהה לו. השורה נקראה גם על ידי המודול עצמו (`readProviderGovernanceEvidence`) והוכרעה `VERIFIED` | סוכן |
| 30.08.2026 | כל חמש | הרצת השער האמיתי על חמש השורות **ללא היתר** | ‏`assertProviderGovernance` → `allowed=true`, `prelaunchException` **נעדר**; ‏`assertGovernedProviderConstruction` עבר גם בלי `organizationId`. בקרה שלילית באותה רתמה: הסרת `dpa` ⇒ `dpa=row_absent`, ‏`status=MISSING` ⇒ `dpa=status_MISSING`, ‏`http://` ⇒ `source_not_a_url`, תאריך `30/08/2026` ⇒ `retrieval_date_malformed`. רתמה שרק אומרת „מותר" אינה ראיה | סוכן |
| 30.08.2026 | — | ‏`AI_ASSISTANT_PRELAUNCH_EXCEPTION` **הוסר** מסודות הייצור | ‏29 סודות במקום 30; חמש שורות הממשל נשארו. **ההיתר לא פג — הוא נמחק**, ולכן `#271` אינו יכול לחזור לפעולה בשקט | סוכן, בהוראת בעלים |
| 30.08.2026 | כל חמש | ‏**האימות שמכריע: הרצה חיה בייצור בלי היתר.** פונקציית `assistant` נפרסה מחדש (v13, ‏`ACTIVE`, ‏`verify_jwt=true`) כדי להבטיח isolate קר שקורא את התצורה החדשה, ואז נשאלו שתי שאלות אמיתיות דרך כניסת `owner@gamos.demo` | ‏**HTTP 200 פעמיים** — ‏`2e1a6aa2…` ו-`faa53696…`, שתיהן `succeeded`, `model=gpt-5.4-2026-03-05`. **זו ההוכחה המכרעת:** בלי היתר בסביבה, `assertProviderGovernance` פותח רק כאשר **כל חמש** השורות `VERIFIED` — תשובה חיה היא לכן מדידה של השער, לא של המסמך | סוכן, בהוראת בעלים |

---

## 6. מה אסור להבטיח, גם כשהשער ייפתח

**מ-30.08.2026 השער פתוח, והרשימה הזו לא התקצרה באף פריט.** ה-DPA שנחתם הוא הסכם עיבוד נתונים
ולא הסכם שימור: הוא אינו נותן אפס-שימור, אינו קובע 30 יום, ואינו מגדיר אזור. **חוזה חתום הוא
הזמנה לנסח בזהירות רבה יותר, לא פחות** — עכשיו יש מסמך שלקוח יכול לבקש לראות.

- **‏`zero retention` בלי חוזה** — ‏`#179`. גם `store: false` אינו זה. **וגם ה-DPA אינו זה:**
  ‏`v.010126` סעיף 3.3 מטיל את הגדרות השימור על הלקוח ואינו מבטיח שימור כלשהו.
