...
> `bank_details` מהישג כל תפקיד לקוח דרך **הרשאת עמודה**; ‏`suppliers_select` **נשאר כפי שהוא**
> בכוונה, אחרת כל embed של ספק בהזמנות ובקבלה היה מחזיר `null`. ‏**(ג)** ‏**תשלום חירום הוסר** —
> ‏`EXECUTE` בוטל (‏`0111`), הפונקציה **לא נמחקה** כי `audit_logs` מפנה אליה.
