...
- **סובלנות היא סכום, ולכן נושאת מטבע ואין לה ברירת מחדל חוצת־מטבעות:** ארבעת מפתחות הסובלנות
  ...
  `private.money_tolerance` בשרת ו־`src/lib/tolerances.ts` בלקוח, לעולם לא ביד. ‏**`null` פירושו
  „אי אפשר להשוות" ואינו מוחלף במספר** (`#288`, `#290`) — אין שער, אין גזירה ממטבע אחר. מטבע
  בשימוש בלי ערך מוצג ב־`/settings` כדורש קביעה, ושדה ריק נשמר כהיעדר ולא כאפס. תנועת בנק
  במטבע כזה **נעצרת**; קליטת מסמך **נכנסת ומפיקה ממצא `warning`** — אסימטריה מכוונת (`#293`).
