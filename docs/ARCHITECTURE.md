...
> **‏⚠️ שלושה מפרטים במסמך הזה מפגרים אחרי העץ — נמדד 01.09.2026 על `d9146bf4`, ולא תוקנו
> כאן כדי לא להעמיד ספירה שלא נבדקה שורה-שורה.** העקרונות, חוזי ה-P0/P1, מטריצת ההרשאות
> וכללי מודל הנתונים **כן** מתוארים נכון; מה שמפגר הן הרשימות:
>
> | מה | מה כתוב כאן | מה נמדד בעץ |
> |---|---|---|
> | רשימת המיגרציות בעץ המקורות | נעצרת ב-`0134`–`0135` | ‏**274 קבצים, `0001`–`0283`** |
> | רשימת `supabase/functions` | תשע פונקציות | ‏**25 תיקיות** (ללא `_shared`) |
> | „מפת מסכים ונתיבים" | ‏28 שורות | ‏**49 נתיבים** בדייר (`src/App.tsx` + `src/lib/routePolicy.ts`) ועוד **11** בקונסולת התפעול (`src/operator/OperatorRoutes.tsx`) |
>
> **מקור האמת הוא העץ**, לא הרשימות האלה: ‏`supabase/migrations/`, ‏`supabase/functions/`,
> ‏`src/App.tsx` ו-`src/lib/routePolicy.ts`. הפער בעיקרו **קדם** למיזוג גלים 0–7 — רשימת
> המיגרציות לא נגעה מאז `04533b85` (14.08.2026) ורשימת הפונקציות מאז `ab1d1aca` (09.08.2026) — והגלים הוסיפו עליו 16 מיגרציות, פונקציית Edge
> אחת (`billing-checkout`) ומסך כניסה מאוחד. עדכון הרשימות הוא עבודה בפני עצמה.
