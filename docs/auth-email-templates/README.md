...
## ה-stack המקומי קורא את אותם הקבצים

‏`supabase/config.toml` מצביע על חמשת הקבצים האלה ב-`[auth.email.template.*]` דרך `content_path`,
ולכן מייל שיוצא מ-Inbucket המקומי נושא בדיוק את אותם קישורים. זה לא נוחות: תבניות ברירת המחדל של
‏GoTrue שולחות `/auth/v1/verify?token=…`, שמחזיר fragment של `#access_token=` — ו-auth-js עם
‏`flowType: pkce` **מסרב** ל-callback כזה במקום להחליף אותו, כך שפיתוח מקומי היה נתקל בדיוק בכשל
שהשינוי הזה בא להסיר. שני הצדדים קוראים את אותם קבצים, ולכן אי אפשר לערוך אחד ולשכוח את השני.
