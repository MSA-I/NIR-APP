...
- **המנגנון חי ואינרטי:** ‏`0161` השאיר `ocr_pages.monthly` ללא הגבלה בכל המסלולים בכוונה. מספר
  העמודים הוא הכרעת בעלים שטרם התקבלה (‏#168), והפעלה היא UPDATE אחד. ‏`p52` מוכיחה את הסירוב
  מול מכסה שנקבעת בתוך הבדיקה עצמה.
