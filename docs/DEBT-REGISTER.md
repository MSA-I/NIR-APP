...
- **מה נסגר (`0162`, הכרעת בעלים 19.08.2026; הוקשח ב-`0169`):** ‏`enqueue_document_processing` בודק עכשיו גם
...
- **החריגה חסומה ומוצהרת:** כל מסמך בר-עיבוד נושא lower bound שרתי של עמוד אחד, ולכן לקוח
  שנמצא בדיוק במכסה נחסם לפני העלאה נוספת. מסמך שכבר עבר את הגבול אינו יכול לעלות יותר
  מ-`ExtractionLimits.max_ai_pages = 20` (‏`worker/ocr/src/limits.py:25`) — הספק לעולם אינו מתבקש
  ליותר. הרע ביותר: מכסה ועוד 19 עמודים, פעם אחת בתקופה (‏519 ב-Free, ‏3.8%).
- **מה תוקן אחרי המיזוג (`0169`):** ‏`0162` שלחה increment ‏`0`, ולכן בדיוק על הגבול עדיין עבר
  מסמך נוסף. בנוסף, retry מקביל של אותו document היה יכול להמתין לנעילת המונה ואז להידחות לפני
  שראה את ה-job שהקריאה הראשונה כבר יצרה. ‏`0169` משתמשת ב-lower bound ‏`1` ומבצעת recheck
  אידמפוטנטי אחרי נעילת המונה; ‏`p52` כוללת interleaving דטרמיניסטי ושתי קריאות DB אמיתיות.
