...
### §55 — תקרת עמודי ה-OCR נספרת ואינה נאכפת
...
- **הצעד הזול הבא:** ספירת עמודים מוקדמת בזמן ההעלאה עבור PDF (‏`0144` כבר מחשב תקרת עמודים
  לחבילות), ואז בדיקה שנייה ב-`enqueue_document_processing` על `ocr_pages.monthly` באותה עסקה
  ובאותו סדר נעילה. לתמונה יחידה הערך הוא 1 ואינו דורש חישוב.
