...
**מוזג ל־`main` ואינו חי (19.08.2026):** ‏PR #77 (המסך הראשון של ארגון ריק), ‏#78 (ארכיטקטורת
הצבע, סבב א'), ‏#79 (סבב ב') ו־#81 (שישה־עשר הליקויים מסקירת הבעלים בנייד).

‏#77, ‏#78 ו־#79 הם frontend בלבד. ‏**#81 אינו** — הוא נושא ארבע מיגרציות שטרם הוחלו ב־Production:
‏`0147_platform_autonomy_policy_read`, ‏`0148_dashboard_due_window_money`,
‏`0149_product_display_name` ו־`0150_product_display_name_read_models`. ראש ה־ledger בייצור נמדד
ב־19.08.2026 והוא `0146`. לכן ה־rollout דורש את **איחוד** שורת ה־Migration ושורת ה־Frontend
במטריצה שב־`CLAUDE.md`, ולא את שורת ה־Frontend לבדה. ‏`ProductNameReview` ומסכי המוצרים
קוראים את `display_name`; פריסת ה־bundle בלי `0149` תחזיר שגיאת עמודה חסרה.
