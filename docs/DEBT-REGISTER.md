...
### §40 — פיצול מחירונים ארוכים בפירוש טרם מומש

- **מצב:** ‏`PROVIDER_TIMEOUT_MS` הועלה מ־45 ל־90 שניות והמעטפת נכפית עכשיו על הקריאה כולה דרך
  `PROVIDER_TOTAL_BUDGET_MS`. מדידות ייצור: מסמך של 22–79 שורות נמשך 33.9–45.1 שניות, כך ש־90
  מכפילות את המרווח. מסמך ארוך בהרבה עדיין יכול לחרוג, וגם `MAX_OUTPUT_TOKENS = 32_768` הוא תקרה.
- **ראיה:** ‏`supabase/functions/interpret-document/core.ts`, הבדיקה „the complete provider retry
  envelope stays inside the database lease with margin” ו־`document_interpretations.duration_ms`.
- **הצעד הבא:** פיצול לפי טווחי עמודים במתכונת `document_packet_segments`, עם מיזוג `line_items`
  שמשמר `evidence_block_ids`. הסף לפיצול נקבע ממדידה, לא מניחוש.
