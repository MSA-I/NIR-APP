...
**מה חסר להפעלה:** ‏`inplace.digital` **נרכש** ב-23.08.2026, אך לא הואצל, לא הוגדר ולא אומת
(§6.א). נדרשים האצלת nameservers ל-Cloudflare, ‏SPF/DKIM/DMARC, אימות Resend, הגדרת השולחים,
SMTP Auth ל־Supabase, החלת `0168`, פריסת `email-sender` ו־smoke חיצוני. בלי תצורה הפונקציה עונה
`misconfigured`; העבודה הידנית והפורטל נשארים זמינים.
...
## 5. דומיין ו־origins — הדומיין נרכש; טרם הואצל, הוגדר או נפרס
...
| ‏TXT ‏(DMARC) | `_dmarc` | `"v=DMARC1; p=none; rua=mailto:<תיבה טרם הוכרעה>;"` — ‏`p=none` **הוכרע**; ראה למטה | — | — | `Auto` |
...
1. דוחות ה-`rua` נאספים ונקראים בפועל לאורך תקופה מייצגת (לא יום אחד).
...
**נשאר פתוח — תיבת ה-`rua`:** לאן נשלחים דוחות ה-aggregate? ‏#237 קובע כתובת שולח יחידה
**בלי `reply-to`**, ואין למוצר תיבה נכנסת. ‏**‏`p=none` בלי יעד `rua` שנקרא בפועל הוא ניטור על
הנייר** — הוא לא ישבור מסירה, אבל גם לא ייצר את הנתונים שתנאי 1–3 למעלה דורשים, ולכן יקפיא את
המדיניות ב-`none` לנצח. זו שאלה פתוחה לבעלים, ואין לבחור לה ערך בשקט.
...
| תיבת `rua` לדוחות DMARC | — | **לא הוכרע.** ‏#237 אוסר `reply-to` ואין תיבה נכנסת. שאלה פתוחה לבעלים |
