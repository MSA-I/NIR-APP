...
**פריסה:** ‏`supabase functions deploy supplier-portal --no-verify-jwt` אחרי החלת `0167` (הפונקציה
קוראת RPCs שהמיגרציה יוצרת — סדר: מסד ואז Edge ואז frontend).
...
**תצפית:** כשלי lookup נרשמים ב-`private.supplier_portal_lookup_failures` (‏30 יום שמורים);
נעילות — ‏`supplier_order_links.locked_until`. משטח קריאה למסך תפעול יתווסף בפאזת התצפיות.
