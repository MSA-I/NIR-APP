...
**מה לא משתנה:** ששת סקריפטי ה-`check:*`, 13 סוויטות ה-SQL, שער ה-Deno, שער הדפדפן ו-`check-p0-security.ps1`
נשארים כפי שהם. הם בודקים דברים שבדיקת יחידה אינה יכולה לבדוק — בידוד דיירים אמיתי, מרוצים אמיתיים,
ודפדפן אמיתי.
...
- `"test": "vitest run"`, ומשולב ל-`build` **אחרי** ששת סקריפטי ה-`check:*` הקיימים.
