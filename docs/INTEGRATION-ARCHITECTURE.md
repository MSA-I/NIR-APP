...
## 7. שירות הרינדור — המשטח השני על ה-VPS (28.08.2026)

`worker/render` מייצר את מסמכי ה-PDF של המוצר. הוא קיים מסיבה אחת שאי אפשר לפתור בדפדפן:
**מסמך שנוצר אצל הלקוח אינו יכול להיות אכוף.** סימן מים שמצויר בקוד שרץ במחשב הקורא הוא מיתוג,
ולא הגבלה. וכל עוד המחולל צילם את המסך, הטקסט במסמך היה תמונה.

**מה שהמעבר לצד שרת פותר, ולמה זה אותו מהלך אחד.** ‏Chromium מסדר את העברית נכון — זו הסיבה
שהמחולל בדפדפן צילם מסך מלכתחילה — ו-`page.pdf()` פולט **טקסט אמיתי** במקום תמונה. אותו מהלך
גם מוציא את ההטבעה מהישג ידו של הלקוח. שני החובות, `DEBT §68` ו-`§69`, נסגרים יחד.

**מה שאין כאן, במכוון: אין תבנית שנייה.** השירות פותח את אותו מסך שהמשתמש רואה, ו-`@media print`
ב-`src/index.css` הוא ה"תבנית" — הוא כבר מחליט מה יורד (`.no-print`), מה מופיע רק על נייר
(`.print-only`) ומה ה-`@page` של הדוח. מסמך שנראה שגוי בשירות נראה שגוי גם במדפסת, וזה הצימוד
הנכון.

| מה | איפה |
|---|---|
| השירות | `worker/render/` — ‏Node + Playwright, ‏`POST /render`, ‏`GET /health` שמחזיר את גרסת החוזה |
| הפריסה | `docker-compose.render.yml`, **ידנית ל-VPS** כמו `worker/ocr`. אף שער ואף merge אינו נוגע בו |
| הצד המזמין | `supabase/functions/render-document` — מאמת את הקורא, פותר את ההרשאה, מזמין, מחזיר PDF |
| גרסת החוזה | `RENDER_CONTRACT_VERSION` בשני הקבצים; שער CI משווה את שני הליטרלים |

**הסודות.** ‏`RENDER_SERVICE_TOKEN` הוא סוד משותף — אותו ערך בדיוק ב-`docker-compose.render.yml`
של ה-VPS וב-secrets של פרויקט Supabase. ‏`RENDER_SERVICE_URL` מצביע על השירות דרך ה-reverse proxy
(ה-container מאזין ל-loopback בלבד). ‏`RENDER_APP_URL` הוא מקור האפליקציה הציבורי. **כל עוד שני
הראשונים חסרים, הפונקציה עונה `renderer_not_configured` והקליינט מייצר את המסמך בעצמו** — מצב
תקין, שבו סימן המים חוזר להיות מיתוג בלבד.

**גבול האמון, מוצהר.** המסך קורא את נתוני הדייר תחת RLS, ולכן השירות מקבל את ה-session של הקורא
— בגוף הבקשה, לא ב-URL, ולשניות שהרינדור נמשך. כלומר לשירות הזה יש באותן שניות את מה שלקורא יש.
ההגנות הן הסוד המשותף, רשימת הנתיבים המותרים שנאכפת **בשני הצדדים**, ו-context של דפדפן שנהרס
אחרי כל בקשה. זה כתוב כאן ולא מתגלה אחר כך.

**המלכודת שכבר עלתה חמישה ימים.** גרסת חוזה שזזה בצד אחד בלבד השביתה את עיבוד המסמכים בשקט
ב-24.08.2026 — העובד דיווח `Up` ונכשל בכל poll. כאן: השער משווה את הליטרלים, השירות עונה
