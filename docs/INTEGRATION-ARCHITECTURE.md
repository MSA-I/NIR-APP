...
| Meilisearch | לא נפרס, **וגם לא נמדד** — החלפת pg_trgm נשארה אסורה בגל 9 במפורש. `SearchProvider` מעל `global_search()` הקיימת (`0011:113`) — pg_trgm + ILIKE, כי לעברית אין מילון PostgreSQL. **מה שכן שונה ב-`0069`: שער סוגי-התוצאה עבר מהדפדפן לשרת** (‏`auth_role()` בוחר ענפים; ‏`payer`/`supplier` אינם מקבלים דבר) — תיקון הרשאה, לא מנוע חיפוש חדש |
