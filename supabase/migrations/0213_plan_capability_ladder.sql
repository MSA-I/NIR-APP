-- 0213 — The subscription ladder becomes an enforceable server contract.
--
-- Decisions: OPEN-DECISIONS #274, #276 and #277.
--
...
values
  ('branches.max', 'numeric', 'current', 'branches', 'סניפים',
   'How many branch units the organization may hold at once.', '0213'),
  ('documents.automatic_monthly', 'numeric', 'per_period', 'documents', 'מסמכים בקריאה אוטומטית',
   'How many documents may enter automatic interpretation in one usage period.', '0213'),
  ('history.full', 'boolean', 'current', null, 'היסטוריה מלאה',
   'Access to closed operational history older than three months.', '0213'),
  ('notifications.email', 'boolean', 'current', null, 'התראות ואוטומציות במייל',
   'Access to plan-scoped operational email notifications and automations.', '0213'),
  ('payments.accountant_queue', 'boolean', 'current', null, 'תור תשלומים לרואה החשבון',
   'Access to the accountant payment execution queue.', '0213'),
  ('invoices.consolidated', 'boolean', 'current', null, 'חשבוניות מרכזות',
   'Access to consolidated supplier invoice intake and reconciliation.', '0213'),
  ('integrations.api', 'boolean', 'current', null, 'חיבור למערכות אחרות',
   'Access to tenant integration and webhook configuration surfaces.', '0213')
...
comment on function public.current_plan_capability(text) is
  'Fail-closed caller-org capability resolver (0213, #274/#276). No organization argument exists.';
...
-- A5, which correctly noticed that 0213 changed both bodies.
insert into private.scope_definer_exemptions (function_signature, reason, target_wave) values
  ('public.effective_entitlement(uuid,text)'::regprocedure::text,
   'internal-only', '0213 plan capability ladder'),
  ('public.get_public_plan_quotas()'::regprocedure::text,
   'public-global-catalogue', '0213 plan capability ladder');
...
begin
  ...
  if v_count <> 0 then
    raise exception '0213: % disabled capability row(s) have no owner decision', v_count;
  end if;
  ...
  if exists (
    ...
    raise exception '0213: the capability ladder is not monotonic';
  end if;

  if (select count(*) from public.get_public_plan_features()) <> 44 then
    raise exception '0213: the public feature catalogue is incomplete';
  end if;
  ...
  if exists (
    ...
    raise exception '0213: Business leaked into the public feature catalogue';
  end if;
  ...
  if not exists (
    ...
    raise exception '0213: a publishable current-count quota has no write guard';
  end if;
  ...
  if not exists (
    ...
    raise exception '0213: the automatic document quota has no runtime consumer';
  end if;
