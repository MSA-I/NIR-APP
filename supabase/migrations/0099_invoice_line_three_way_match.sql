-- 0099 -- Immutable invoice-line evidence and server-authoritative three-way matching.
--
...
-- Tenant read-only latch for the six tables this migration adds. Same reason as 0096: 0092's
-- one-time information_schema sweep ran before these tables existed, so each needs the trigger
-- stated explicitly. These six carry invoice line evidence and three-way approval snapshots --
-- financial records -- so an unguarded write path here is the costliest of the set.
...
-- 0095 requires every definer that touches a scoped financial table to pin its reviewed live
...
from (values
  (
    ...
    '0099 locks the exact invoice; browser actors are filtered by tenant and null-or-auth_scopes unit, while service ingestion requires an exact tenant document/interpretation/actor chain.'
  ),
  (
    'record_invoice_line_matches(uuid,uuid,uuid,uuid,jsonb,text)',
    '0099 locks the exact tenant invoice through a null-or-auth_scopes unit predicate before validating every explicit allocation against linked supplier purchase orders.'
  ),
  (
    'override_invoice_three_way_match(uuid,text,uuid,text)',
    '0099 locks the exact tenant invoice through a null-or-auth_scopes unit predicate and rechecks owner identity, scope and fresh password after serialization.'
  ),
  (
    'get_invoice_three_way_match(uuid)',
    '0099 exposes private assessment helpers only after filtering the exact invoice by auth_org, role and the canonical null-or-auth_scopes legal-unit predicate.'
  )
...
begin
  ...
  if v_violations is not null then
    raise exception e'0099 scope assertions failed:\n%', v_violations;
  end if;
end
$$;
