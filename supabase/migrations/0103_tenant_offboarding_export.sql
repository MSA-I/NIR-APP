-- 0103 -- Tenant offboarding, full export evidence and conservative retention boundaries.
--
...
begin
  ...
  if md5(replace(v_body, e'\r', '')) <> '01804c827bebb9e42af4bc6494fb84b8' then
    raise exception '0103 ancestry guard failed: organization_access_mode changed';
  end if;

  select p.prosrc into v_body
  from pg_catalog.pg_proc p
  where p.oid = 'private.organization_row_write_guard()'::regprocedure;
  if md5(replace(v_body, e'\r', '')) <> '9411a628caa7fb69998255dfdb41260c' then
    raise exception '0103 ancestry guard failed: organization_row_write_guard changed';
  end if;
end
$guard$;
...
-- 0092's table-wide latch also protects offboarding. Its exception is a transaction-local org
...
declare
  v_hash text;
begin
  select md5(replace(procedure.prosrc, e'\r', ''))
  into v_hash
  from pg_catalog.pg_proc procedure
  join pg_catalog.pg_namespace namespace on namespace.oid = procedure.pronamespace
  where namespace.nspname = 'public'
    and procedure.oid = 'public.smart_document_extraction_valid(jsonb,text)'::regprocedure;
  if v_hash is distinct from 'ba079ba5e2c39e4de3a57a3275817e4a' then
    raise exception '0103 ancestry guard failed: smart_document_extraction_valid changed';
  end if;
end
$guard$;
...
-- bank details. 0097 rebuilt this policy for the accountant projection and accidentally restored
...
begin
  ...
  if v_supplier_policy is null
    ...
    raise exception
      '0103 supplier raw-row assertion failed: supplier agents must use supplier_portal_context()';
  end if;
