-- 0257 -- The legal hold gets a way off. (Owner decision #301, 30.08.2026.)
--
...
--
-- THE RULING (#301, 30.08.2026), with the alternatives the owner rejected recorded here so that a
...
--
-- WHAT THIS FILE DELIBERATELY DOES NOT BUILD: a command that PLACES a hold. #301 is about the
...
--   super_admin  -- the owner named this role and only this role (#301).
...
comment on function public.release_organization_legal_hold(uuid, text) is
  'Lifts the legal hold on a customer organization''s live offboarding request (#301, 0257). '
...
begin
  ...
  if position('reason_required' in v_preamble) = 0 then
    raise exception '0257: private.assert_platform_command no longer refuses a blank reason, so '
                    'the mandatory reason #301 asked for is not enforced anywhere';
  end if;
  ...
  if position('offboarding.handle' in v_body) > 0 then
    raise exception '0257: the release command names offboarding.handle, which customer_ops '
                    'holds -- #301 named super_admin alone';
  end if;
