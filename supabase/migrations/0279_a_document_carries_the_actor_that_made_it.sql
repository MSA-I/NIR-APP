-- 0279 — a document carries the actor that made it, and a machine is an actor.
--
...
begin
  ...
  if v_orphans > 0 then
    raise exception using
      errcode = '23514',
      message = format('0279 preflight: %s document(s) have no uploader', v_orphans),
...
begin
  ...
  if v_source is null then
    raise exception '0279: public.service_materialize_document_packet is not defined';
  end if;
  ...
  select (length(v_source) - length(replace(v_source, 'document_kind,supplier_id,document_date,unit_id', '')))
    ...
    into v_hits;
  if v_hits <> 1 then
    raise exception '0279: expected exactly one packet child-document column list, found %', v_hits;
  end if;
  ...
  if v_patched = v_source then
    raise exception '0279: the packet child-document patch produced no change';
  end if;
  execute v_patched;
end
$packet_0279$;
...
begin
  ...
  if v_source is null then
    raise exception '0279: private.claim_document_interpretation_jobs is not defined';
  end if;
  ...
  select (length(v_source) - length(replace(v_source, 'join public.profiles p', ''))) / length('join public.profiles p')
    into v_hits;
  if v_hits <> 1 then
    raise exception '0279: expected exactly one profiles join in the live claimer, found %', v_hits;
  end if;
  if position('left join public.profiles p' in v_source) > 0 then
    raise exception '0279: the live claimer already left-joins profiles; the anchor is not what this migration read';
  end if;
  select (length(v_source) - length(replace(v_source, 'j.requested_by = d.uploaded_by', ''))) / length('j.requested_by = d.uploaded_by')
    into v_hits;
  if v_hits <> 1 then
    raise exception '0279: expected exactly one requested_by pin in the live claimer, found %', v_hits;
  end if;
  ...
  if v_patched = v_source then
    raise exception '0279: the claimer patch produced no change';
  end if;
  execute v_patched;
end
$claimer_0279$;
...
begin
  ...
  if (select count(*) from information_schema.columns
    ...
    raise exception '0279: source is missing from one of the three tables';
  end if;
  ...
  if (select count(*) from pg_constraint
    ...
    raise exception '0279: an actor/source CHECK is missing';
  end if;
  ...
  if (select count(*) from pg_constraint
    ...
    raise exception '0279: a job/document source FK is missing';
  end if;
  ...
  if v_bad > 0 then
    raise exception '0279: requested_by is still NOT NULL on % work table(s)', v_bad;
  end if;
  ...
  if (select count(*) from pg_catalog.pg_trigger
    ...
    raise exception '0279: a work row can be written with a source of its writer''s choosing';
  end if;
  ...
  if position('v_parent.document_date,v_parent.unit_id,v_parent.source' in
    ...
    raise exception '0279: a packet split still gives its children the default source';
  end if;
  ...
  if not has_function_privilege('service_role',
    ...
    raise exception '0279: the intake worker cannot reserve or release quota';
  end if;
  if has_function_privilege('authenticated',
        'private.inbound_reserve_quota(uuid,text,numeric)', 'execute') then
    raise exception '0279: a browser role can reserve intake quota';
  end if;
  ...
  if (select position('d.source <> ''browser'' and j.requested_by is null' in
    ...
    raise exception '0279: the interpretation claimer still requires a human actor';
  end if;
  ...
  if (select count(*) from private.inbound_channel_boundary where enabled) > 0 then
    raise exception '0279: an intake channel is enabled at migration time';
  end if;
  if has_table_privilege('service_role', 'private.inbound_channel_boundary', 'UPDATE')
    ...
    raise exception '0279: a product role can write the platform intake boundary';
  end if;
  ...
  if not has_function_privilege('service_role',
    ...
    raise exception '0279: service_role cannot execute the ingest command it is the only caller of';
  end if;
  if has_function_privilege('authenticated',
        'public.service_ingest_inbound_document(uuid,uuid,uuid,text)', 'execute')
     or has_function_privilege('anon',
        'public.service_ingest_inbound_document(uuid,uuid,uuid,text)', 'execute') then
    raise exception '0279: a browser role can execute the ingest command';
  end if;
  ...
  if (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    ...
    raise exception '0279: the ingest RPC does not have the four-argument signature';
  end if;
