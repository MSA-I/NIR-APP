-- 0247: the exported PDF learns which plan it belongs to.
--
...
comment on function public.my_export_watermark() is
  ...
  '(0247). Resolves the exports.unbranded_pdf entitlement through override then plan, and refuses '
...
begin
  ...
  if v_free is distinct from false then
    raise exception '0247: the free rung must not be granted an unbranded export';
  end if;
  ...
  if v_paid > 0 then
    raise exception '0247: % rung(s) other than free lost the unbranded export grant', v_paid;
  end if;
  ...
  if v_violations is not null then
    raise exception e'0247 scope assertions failed:\n%', v_violations;
  end if;

  select string_agg(detail, e'\n' order by detail)
    into v_violations from private.tenant_export_registry_violations();
  if v_violations is not null then
    raise exception e'0247 tenant export assertions failed:\n%', v_violations;
  end if;
end
$assert_0247$;
