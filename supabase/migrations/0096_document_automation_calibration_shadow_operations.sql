-- 0096 -- Measured price-list automation: immutable shadow predictions, human calibration
...
-- Tenant read-only latch for the five tables this migration adds.
--
-- 0092 attaches this trigger with a one-time loop over information_schema, so it covers only
-- the tables that existed when it ran. In the campaign's own ordering this migration came
-- first and the loop swept it up; adopting the lifecycle latch early as 0092 inverted that,
-- and every table below now lands after the sweep. Without these five statements an expired
-- or offboarding tenant could still write automation evidence through a definer or
-- service_role path, and p22_trial_read_only's universal assertion would fail.
...
begin
  if has_function_privilege(
    ...
    raise exception '0096 refused: browser can execute shadow evaluation';
  end if;
  if has_function_privilege(
       'service_role',
    ...
    raise exception '0096 refused: service role can bypass scope eligibility';
  end if;
  if has_function_privilege(
       'service_role',
    ...
    raise exception '0096 refused: service role can author human calibration evidence';
  end if;
  if position(
    ...
    raise exception '0096 refused: platform automation eligibility lacks step-up enforcement';
  end if;
  if has_table_privilege('service_role', 'public.price_list_shadow_runs', 'INSERT')
    ...
    raise exception '0096 refused: service role can bypass the ledger RPC boundaries';
  end if;
