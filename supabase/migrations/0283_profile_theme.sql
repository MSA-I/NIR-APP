-- 0283 -- the appearance a person chose, stored per person.
--
-- Owner ruling #8 of 31.08.2026 (OPEN-DECISIONS #331, ADR-0010): the light/dark choice persists
...
begin
  ...
  if position('''theme''' in v_def) = 0 then
    ...
    if v_array_old is null then
      raise exception '0283: the browser-writable allow-list is no longer an array literal '
    ...
    if v_hits <> 2 then
      raise exception '0283: expected the browser-writable allow-list twice in '
    ...
    if v_hits <> 1 then
      raise exception '0283: expected the allow-list comment once, found % -- re-read the live body',
        v_hits;
    end if;
    ...
    if position(v_self_fields in v_def) = 0 then
      raise exception '0283: the allow-list comment still does not name [%] -- the replacement did '
...
begin
  if not exists (
    ...
    raise exception '0283: profiles.theme was not created';
  end if;
  ...
  if not exists (
    ...
    raise exception '0283: the supported-theme constraint is missing';
  end if;

  v_def := replace(pg_get_functiondef(
    'public.profiles_guard_privileged_columns()'::regprocedure), e'\r', '');
  if position('''theme''' in v_def) = 0 then
    raise exception '0283: the column guard still treats theme as privileged, so no browser could '
  ...
  if not has_column_privilege('authenticated', 'public.profiles', 'theme', 'update') then
    raise exception '0283: authenticated cannot UPDATE profiles.theme -- 0042''s column ACL still '
  ...
  if v_granted is distinct from
    ...
    raise exception '0283: the profiles self-service ACL is now [%], not 0042''s five columns plus '
  ...
  begin
    begin
      ...
      if found then
        raise exception '0283: the constraint accepted an unsupported theme';
      end if;
      raise exception '0283_probe_rollback';
    exception when check_violation then
      raise exception '0283_probe_rollback';
    end;
  exception when others then
    if sqlerrm <> '0283_probe_rollback' then raise; end if;
  end;
  ...
  if v_violations is not null then
    raise exception e'0283 scope assertions failed:\n%', v_violations;
  end if;

  select string_agg(detail, e'\n' order by detail)
    into v_violations from private.tenant_export_registry_violations();
  if v_violations is not null then
    raise exception e'0283 tenant export assertions failed:\n%', v_violations;
  end if;
end
$assert_0283$;
