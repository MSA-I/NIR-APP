-- 0216 -- the capability ladder the owner decided on 28.08.2026, and the server half that makes it
...
-- later ruling wins. `#278` records it. Everything on the list moves to `פרו`:
--
...
-- record saying who decided it or against what. Same shape, same duty.
create table if not exists private.plan_capability_decisions (
...
comment on table private.plan_capability_decisions is
  'Why a boolean capability is off for a plan (0216, #278). The assertion at the end of this file '
  'refuses any plan_entitlements row set to false without a matching decision here -- the boolean '
  'counterpart of private.plan_quota_decisions, and the brake #274 requires.';

insert into private.plan_capability_decisions (plan_key, entitlement_key, included, decision_ref, note)
values
  ('free',  'reports.advanced',    false, '#278',
   'הדוח לרואה חשבון ולוח ביצועי הספקים נפתחים במסלול פרו (הכרעת בעלים 28.08.2026, גוברת על #274)'),
  ('basic', 'reports.advanced',    false, '#278',
   'הדוח לרואה חשבון ולוח ביצועי הספקים נפתחים במסלול פרו (הכרעת בעלים 28.08.2026, גוברת על #274)'),
  ('free',  'bank.reconciliation', false, '#278',
   'התאמות בנק הן שכבת רואה החשבון ונשארות פרו, כפי ש-#274 כבר קבע'),
  ('basic', 'bank.reconciliation', false, '#278',
   'התאמות בנק הן שכבת רואה החשבון ונשארות פרו, כפי ש-#274 כבר קבע'),
  ('free',  'exports.custom',      false, '#278',
   'תבניות הייצוא וחוברות ה-Excel נפתחות במסלול פרו (הכרעת בעלים 28.08.2026, גוברת על #274)'),
  ('basic', 'exports.custom',      false, '#278',
   'תבניות הייצוא וחוברות ה-Excel נפתחות במסלול פרו (הכרעת בעלים 28.08.2026, גוברת על #274)')
...
comment on function private.auth_org_allows(text) is
  'Whether the caller''s own plan includes a boolean capability (0216, #278). A live override wins '
...
comment on function private.plan_capability_write_guard() is
  'Refuses a tenant write to a table whose capability the plan does not include (0216, #278). '
...
comment on function private.invitation_seat_guard() is
  'Refuses an invitation that would take an organization past its users.max (0216, #278; closes '
...
begin
  ...
  if v_undecided <> 0 then
    raise exception '0216: % capability(ies) are off for a plan with no decision row', v_undecided;
  end if;
  ...
  if v_break is not null then
    raise exception e'0216 monotonicity broken:\n%', v_break;
  end if;
  ...
  if v_free_bank is not false or v_pro_bank is not true then
    raise exception '0216: bank.reconciliation resolved free=%, pro=%', v_free_bank, v_pro_bank;
  end if;
  if (select numeric_limit from plan_entitlements
       where plan_key = 'basic' and entitlement_key = 'users.max') <> 1 then
    raise exception '0216: basic still carries more than one member seat';
  end if;
  ...
  if exists (
    ...
    raise exception '0216: an existing organisation would lose bank reconciliation on apply';
  end if;
  ...
  if v_violations is not null then
    raise exception e'0216 scope assertions failed:\n%', v_violations;
  end if;
end
$assert_0216$;
