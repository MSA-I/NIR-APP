-- 0098 -- Safe tenant branding: the existing organization name plus one constrained logo.
...
begin
  ...
  if md5(replace(v_body, e'\r', '')) <> 'f5da4a0c56372b6acec17ef3ceca67ad' then
    raise exception '0098 ancestry guard failed: organizations_guard_lifecycle changed';
  end if;

  select p.prosrc into v_body
  from pg_catalog.pg_proc p
  where p.oid = 'private.audit_organizations_change()'::regprocedure;
  if md5(replace(v_body, e'\r', '')) <> '8dce4a4986fc9965c6f24cf17eb60961' then
    raise exception '0098 ancestry guard failed: audit_organizations_change changed';
  end if;
end
$$;
...
begin
  ...
  if v_violations is not null then
    raise exception e'0098 scope assertions failed:\n%', v_violations;
  end if;
end
$$;
