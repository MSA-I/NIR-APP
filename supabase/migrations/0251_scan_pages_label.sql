-- 0251: the page quota stops calling itself OCR.
--
...
begin
  ...
  if v_label is null then
    raise exception '0251: ocr_pages.monthly is not in the definitions at all';
  end if;
  if v_label <> 'עמודי סריקה בחודש' then
    raise exception '0251: the page quota is still labelled "%"', v_label;
  end if;
  ...
  if exists (select 1 from private.entitlement_definitions where label ilike '%OCR%') then
    raise exception '0251: a tenant-facing entitlement label still names OCR';
  end if;
  ...
  if v_violations is not null then
    raise exception e'0251 scope assertions failed:\n%', v_violations;
  end if;
end
$assert_0251$;
