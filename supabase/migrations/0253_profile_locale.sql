-- 0253 -- the interface language a person chose, stored per person.
--
...
begin
  ...
  if position('''locale''' in v_def) = 0 then
    ...
    if v_hits <> 2 then
      raise exception '0253: expected the browser-writable allow-list twice in '
    ...
    v_hits := (length(v_def) - length(replace(v_def, v_anchor, ''))) / length(v_anchor);
    if v_hits <> 1 then
      raise exception '0253: expected the allow-list comment once, found % -- re-read the live body', v_hits;
    end if;
    v_def := replace(v_def, v_anchor, v_replacement);
...
begin
  ...
  if not exists (
    ...
    raise exception '0253: profiles.locale was not created';
  end if;
  ...
  if not exists (
    ...
    raise exception '0253: the supported-locale constraint is missing';
  end if;

  v_def := replace(pg_get_functiondef(
    'public.profiles_guard_privileged_columns()'::regprocedure), e'\r', '');
  if position('''locale''' in v_def) = 0 then
    raise exception '0253: the column guard still treats locale as privileged, so no browser '
  ...
  if not has_column_privilege('authenticated', 'public.profiles', 'locale', 'update') then
    raise exception '0253: authenticated cannot UPDATE profiles.locale -- 0042''s column ACL still '
  ...
  if (select count(*) from information_schema.column_privileges
    ...
    raise exception '0253: the profiles self-service ACL is no longer exactly 0042''s five columns '
  ...
  begin
    begin
      ...
      if found then
        raise exception '0253: the constraint accepted an unsupported locale';
      end if;
      raise exception '0253_probe_rollback';
    exception when check_violation then
      raise exception '0253_probe_rollback';
    end;
  exception when others then
    if sqlerrm <> '0253_probe_rollback' then raise; end if;
  end;
  ...
  if v_violations is not null then
    raise exception e'0253 scope assertions failed:\n%', v_violations;
  end if;

  select string_agg(detail, e'\n' order by detail)
    into v_violations from private.tenant_export_registry_violations();
  if v_violations is not null then
    raise exception e'0253 tenant export assertions failed:\n%', v_violations;
  end if;
end
$assert_0253$;
