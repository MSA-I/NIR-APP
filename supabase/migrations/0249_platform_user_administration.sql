-- 0249 -- User administration for the platform console.
--
...
values
  ('user.view',       'Read the cross-tenant user directory and one user''s detail.',      'read', false, '0249'),
  ('user.access',     'Change a tenant user''s role, or suspend and restore their access.', 'high', true,  '0249'),
  ('operator.manage', 'Add or remove a platform operator and change the roles they hold.',  'high', true,  '0249');
...
comment on table platform_admin_events is
  'Append-only history of the platform operator roster (0249): who granted or revoked whose '
...
comment on function public.platform_users(text, uuid, text, text, integer, integer) is
  'Cross-tenant user directory for the operator console (0249). Requires user.view; zero rows '
...
comment on function public.platform_user_detail(uuid) is
  'One tenant user, as the operator console shows them (0249). org_owner_count rides along '
...
comment on function public.platform_user_events(uuid, integer) is
  'What the platform did to one tenant user, read back from platform_lifecycle_events (0249). '
...
begin
  ...
  if v_src is null then
    raise exception '0249: public.profiles_guard_privileged_columns() not found';
  end if;
  ...
  if v_occurrences <> 1 then
    raise exception '0249: expected exactly 1 owner-handshake block in profiles_guard_privileged_columns, found %',
      v_occurrences;
  end if;
...
comment on function public.platform_set_user_access(uuid, user_role, boolean, text) is
  'Change a tenant user''s role or suspend them, from the platform console (0249). Requires '
...
comment on function public.platform_operator_events(integer) is
  'History of the operator roster (0249). Readable by any operator -- who holds authority here '
...
comment on function public.platform_user_overview() is
  'The operator dashboard''s headline counts (0249). Zero rows without user.view, so the console '
...
begin
  ...
  if v_violations is not null then
    raise exception e'0249 scope assertions failed:\n%', v_violations;
  end if;
end
$assert_0249$;
...
begin
  ...
  if v_count > 0 then
    raise exception '0249: super_admin is missing % of the three new capabilities', v_count;
  end if;
  ...
  if v_count > 0 then
    raise exception '0249: operator.manage leaked to % non-super_admin role(s)', v_count;
  end if;
  ...
  if not exists (
    ...
    raise exception '0249: the profiles guard did not take the platform handshake';
  end if;
  ...
  if exists (select 1 from public.platform_users()) then
    raise exception '0249: platform_users returned rows with no JWT subject';
  end if;
  if exists (select 1 from public.platform_user_overview()) then
    raise exception '0249: platform_user_overview returned rows with no JWT subject';
  end if;
  if exists (select 1 from public.platform_operator_events()) then
    raise exception '0249: platform_operator_events returned rows with no JWT subject';
  end if;
end
$anchor_0249$;
