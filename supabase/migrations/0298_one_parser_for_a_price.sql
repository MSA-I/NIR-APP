-- 0298 — one parser for a price, and the currency the writer never named. Wave 2, RC2.
--
-- THE DEFECT, AND WHY IT IS TWO DEFECTS. Three live bodies strip a price with
-- `regexp_replace(..., '[[:space:]₪,]', '', 'g')`, which removes a shekel sign and a comma and
-- leaves everything else, so `$12.50` reaches `::numeric` as `$12.50` and dies as an unreadable
-- row rather than as a dollar one. And a fourth, the PREVIEW, strips with `[^0-9.]` — the one
-- place in the whole history carrying that expression, and NOT a corrected version of the others:
-- it destroys the sign (`-5` becomes `5`) and swallows letters (`12.5USD` becomes `12.5`), so the
-- preview accepts rows all three writers then refuse. It is replaced here, not adopted.
--
-- THE CURRENCY GAP IS ONE FUNCTION DEEPER than the three parsers, and that is the half a reader
-- of the parser alone would miss. All three hand their rows to
-- `p1_import_supplier_prices_internal`, and THAT is the writer: it names no currency column when
-- it inserts into `supplier_products` and `price_history`, so both take the ILS default. A dollar
-- price list, correctly read, was still stored in shekels.
--
-- PER-ROW REJECTION IS KEPT, DELIBERATELY. The three writers mark a bad row and continue today. A
-- parser that RAISED would turn a partial intake into a total failure — a supplier whose sheet has
-- one unreadable cell would lose the other four hundred rows. So `parse_price` never raises; it
-- returns `{ok, value, reason, ...}` and the caller decides.
--
-- A CURRENCY CHANGE ON AN EXISTING ROW IS REFUSED, and this was decided in the plan rather than
-- left to implementation. `supplier_products` is `unique (supplier_id, product_id)` with a single
-- `current_price`, a single `previous_price` and one currency column, so the pair holds exactly
-- one currency and a USD list arriving for an ILS-priced product has NO representable outcome.
-- The writer rejects that row with `currency_mismatch_existing_price`: it does not convert, does
-- not overwrite the currency, and does not attempt a second row. Changing the currency a supplier
-- trades in is a deliberate act with its own migration path, and that path is out of this plan.
--
...
-- =====================================================================================
-- 6b. RE-PIN THE TWO BODY HASHES THESE PATCHES INVALIDATED
--
-- This schema pins definer bodies in TWO registries and they answer different questions:
--
...
--                                                          path that was reviewed?
--
-- Each stores the md5 of the body it was reviewed against, so ANY rewrite leaves the registration
-- stale and the scope assertions refuse the migration. That is the guard working -- it is how a
-- body that quietly lost its tenant fence gets caught -- and re-pinning is the deliberate act of
-- saying the new body was looked at. It is also the step this migration was FIRST WRITTEN WITHOUT,
-- and the assertions caught it: `A5 -- stale scope enforcement registration`.
--
-- Neither fence changes here. Both patches replace a price-parsing expression and thread a
-- currency through; they touch no tenant filter, no role check and no unit scope, and the
-- postflight below asserts the parser is the one being called in each body it rewrote.
--
-- The hash is computed with the carriage returns stripped, exactly as the reader computes it, so
-- a body applied from Windows and one applied on a Linux runner pin to the same value.
-- =====================================================================================

update private.scope_definer_enforcements
   set body_hash = (select md5(replace(p.prosrc, chr(13), ''))
                    from pg_proc p
                    where p.oid = 'public.get_qualified_product_creation_dry_run(uuid)'::regprocedure),
       scope_proof = scope_proof
         || ' 0298 replaces the preview''s [^0-9.] strip -- which destroyed the sign and swallowed '
         || 'letters, so the preview accepted rows all three writers refused -- with the shared '
         || 'parser. No tenant, role or unit fence is touched.'
 where function_signature = 'get_qualified_product_creation_dry_run(uuid)';

update private.document_automation_authoritative_functions
   set body_hash = (select md5(replace(p.prosrc, chr(13), ''))
                    from pg_proc p
                    where p.oid = 'public.apply_price_list_interpretation_qualified_impl(uuid,uuid,uuid)'::regprocedure)
 where function_signature = 'apply_price_list_interpretation_qualified_impl(uuid,uuid,uuid)';

do $assert_repin_0298$
begin
  if exists (
    select 1 from private.scope_definer_enforcements e
    where e.function_signature = 'get_qualified_product_creation_dry_run(uuid)'
      and e.body_hash <> (select md5(replace(p.prosrc, chr(13), '')) from pg_proc p
                          where p.oid = e.function_signature::regprocedure)
  ) then
    raise exception '0298: the preview body was rewritten and its tenancy pin was not moved';
  end if;
  if exists (
    select 1 from private.document_automation_authoritative_functions a
    where a.function_signature = 'apply_price_list_interpretation_qualified_impl(uuid,uuid,uuid)'
      and a.body_hash <> (select md5(replace(p.prosrc, chr(13), '')) from pg_proc p
                          where p.oid = a.function_signature::regprocedure)
  ) then
    raise exception '0298: the automatic path was rewritten and its automation pin was not moved';
  end if;
end
$assert_repin_0298$;
