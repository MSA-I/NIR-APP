-- 0215 -- Inviting an operator, without inventing a business for them.
--
-- THE HOLE 0214 LEFT, stated plainly because it is the reason this file exists: the roster gained
...
comment on table platform_operator_invitations is
  'Pending and historical invitations to join the platform operator roster (0215). No org_id: '
...
begin
  if private.platform_invitation_window() <> interval '15 minutes' then
    raise exception '0215: the invitation window is not the fifteen minutes the owner decided';
  end if;
  ...
  if exists (select 1 from public.platform_operator_invitations()) then
    raise exception '0215: the invitation queue answered with no JWT subject';
  end if;
  if (public.lookup_platform_operator_invitation(repeat('a', 64)) ->> 'status') <> 'unknown' then
    raise exception '0215: an unmatched token did not answer unknown';
  end if;
  if (public.lookup_platform_operator_invitation(null) ->> 'status') <> 'unknown' then
    raise exception '0215: a null token did not answer unknown';
  end if;
end
$anchor_0215$;
