...
create function pg_temp.p85_assert(p_condition boolean, p_message text)
...
create function pg_temp.p85_as(p_user uuid, p_fresh_password boolean default false)
returns void language plpgsql as $$
begin
...
create function pg_temp.p85_refused(p_sql text, p_expected text)
returns void language plpgsql as $$
begin
  begin
    execute p_sql;
...
insert into auth.users (id, email) values
  ('79100000-0000-4000-8000-000000000001', 'owner-a-p85@example.test'),
  ('79100000-0000-4000-8000-000000000002', 'office-a-p85@example.test'),
  ('79100000-0000-4000-8000-000000000003', 'owner-b-p85@example.test'),
  ('79100000-0000-4000-8000-000000000004', 'super-p85@example.test'),
  ('79100000-0000-4000-8000-000000000005', 'support-p85@example.test'),
  ('79100000-0000-4000-8000-000000000006', 'second-super-p85@example.test'),
  ('79100000-0000-4000-8000-000000000007', 'not-an-operator-p85@example.test');
...
-- ===== 3. The read door (G2) =====
select pg_temp.p85_as('79100000-0000-4000-8000-000000000004');
select pg_temp.p85_assert(
...
select pg_temp.p85_assert(
  (select count(*) from public.platform_users(p_search => 'office-a-p85')) = 1,
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000001');
select pg_temp.p85_assert(
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000005', true);
select pg_temp.p85_assert(
  (select count(*) from public.platform_users(p_search => 'p85@example.test')) = 3,
  'support cannot read the directory it is supposed to answer questions from');
select pg_temp.p85_refused(
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000004', false);
select pg_temp.p85_refused(
  $$select public.platform_set_user_access(
      '79100000-0000-4000-8000-000000000002', 'accountant', true, 'P79 stale authentication')$$,
  'fresh_authentication_required');

select pg_temp.p85_as('79100000-0000-4000-8000-000000000004', true);
select pg_temp.p85_refused(
  $$select public.platform_set_user_access(
...
select pg_temp.p85_refused(
  $$select public.platform_add_operator(
      'not-an-operator-p85@example.test', 'P79', 'support', 'P79 support attempt')$$,
  'not_platform_capability');
...
select pg_temp.p85_refused(
  $$select public.platform_add_operator(
      'nobody-p85@example.test', 'P79', 'support', 'P79 unknown account')$$,
  'operator_account_unknown');

select public.platform_add_operator(
  'not-an-operator-p85@example.test', 'P79 new operator', 'support', 'P79 onboarding a colleague');
select pg_temp.p85_assert(
...
select pg_temp.p85_refused(
  $$select public.platform_add_operator(
      'not-an-operator-p85@example.test', 'P79', 'support', 'P79 twice')$$,
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000007', true);
select pg_temp.p85_refused(
  $$select public.platform_set_operator_roles(
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000004', true);
select public.platform_add_operator(
  'second-super-p85@example.test', 'P79 second super', 'super_admin', 'P79 succession');

select pg_temp.p85_as('79100000-0000-4000-8000-000000000006', true);
select public.platform_set_operator_roles(
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000005', true);
select pg_temp.p85_refused(
  $$select public.platform_invite_operator('newcomer-p85@example.test', 'support', 'P79 support attempt')$$,
  'not_platform_capability');
...
select pg_temp.p85_refused(
  ...
  'invalid_email');
select pg_temp.p85_refused(
  $$select public.platform_invite_operator('newcomer-p85@example.test', 'no_such_role', 'P79 bad role')$$,
  'platform_role_unknown');
-- Someone already on the roster is changed, not invited — and the product says which command.
select pg_temp.p85_refused(
  $$select public.platform_invite_operator('support-p85@example.test', 'support', 'P79 already ours')$$,
  'operator_already_exists');

-- The happy path. The raw token leaves the database exactly here.
create temporary table p85_invite as
select public.platform_invite_operator(
  'newcomer-p85@example.test', 'support', 'P79 onboarding a colleague') as payload;

select pg_temp.p85_assert(
  (select payload ->> 'email' from p85_invite) = 'newcomer-p85@example.test',
  'the invitation did not come back for the address it was issued to');
select pg_temp.p85_assert(
  (select length(payload ->> 'token') from p85_invite) = 64,
...
select pg_temp.p85_assert(
  not exists (
    ...
    where token_hash = (select payload ->> 'token' from p85_invite)),
...
select pg_temp.p85_assert(
  (public.lookup_platform_operator_invitation((select payload ->> 'token' from p85_invite)) ->> 'status') = 'valid',
  'a live invitation did not read as valid to the person holding the link');
select pg_temp.p85_assert(
  (public.lookup_platform_operator_invitation((select payload ->> 'token' from p85_invite)) ->> 'email')
    = 'newcomer-p85@example.test',
...
insert into auth.users (id, email) values
  ('79100000-0000-4000-8000-000000000008', 'newcomer-p85@example.test'),
  ('79100000-0000-4000-8000-000000000009', 'stranger-p85@example.test');

select pg_temp.p85_as('79100000-0000-4000-8000-000000000009', true);
select pg_temp.p85_refused(
  format($$select public.accept_platform_operator_invitation(%L)$$,
         (select payload ->> 'token' from p85_invite)),
  'invitation_email_mismatch');

select pg_temp.p85_as('79100000-0000-4000-8000-000000000008', true);
select public.accept_platform_operator_invitation((select payload ->> 'token' from p85_invite));
...
select pg_temp.p85_as('79100000-0000-4000-8000-000000000006', true);
create temporary table p85_invite_two as
select public.platform_invite_operator(
  'second-newcomer-p85@example.test', 'analyst', 'P79 second colleague') as payload;
update platform_operator_invitations set expires_at = now() - interval '1 minute'
 where email = 'second-newcomer-p85@example.test';
select pg_temp.p85_assert(
  (public.lookup_platform_operator_invitation((select payload ->> 'token' from p85_invite_two)) ->> 'status')
    = 'expired',
  'a window that has passed did not read as expired');

insert into auth.users (id, email)
values ('79100000-0000-4000-8000-000000000010', 'second-newcomer-p85@example.test');
select pg_temp.p85_as('79100000-0000-4000-8000-000000000010', true);
select pg_temp.p85_refused(
  format($$select public.accept_platform_operator_invitation(%L)$$,
         (select payload ->> 'token' from p85_invite_two)),
  'invitation_expired');

select pg_temp.p85_as('79100000-0000-4000-8000-000000000006', true);
create temporary table p85_invite_three as
select public.platform_invite_operator(
  'second-newcomer-p85@example.test', 'analyst', 'P79 reissue') as payload;
select pg_temp.p85_assert(
  (select payload ->> 'token' from p85_invite_three)
    is distinct from (select payload ->> 'token' from p85_invite_two),
...
select public.platform_revoke_operator_invitation(
  (select id from platform_operator_invitations
    where email = 'second-newcomer-p85@example.test' and accepted_at is null and revoked_at is null),
  'P79 the colleague declined');
select pg_temp.p85_assert(
  (public.lookup_platform_operator_invitation((select payload ->> 'token' from p85_invite_three)) ->> 'status')
    = 'revoked',
  'a revoked invitation did not say so');
select pg_temp.p85_refused(
  format($$select public.platform_revoke_operator_invitation(%L, 'P79 twice')$$,
         (select id from platform_operator_invitations where email = 'second-newcomer-p85@example.test')),
