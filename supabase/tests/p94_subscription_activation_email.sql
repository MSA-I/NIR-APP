-- p94 -- the activation email is owed exactly once, and a settlement cannot be replayed into two.
--
...
begin
  ...
  values ('paddle', 'evt_p94_one', 'subscription.activated', 'stored', '{}'::jsonb, v_org)
  returning id into v_event;
  insert into private.billing_events (provider, provider_event_id, event_type, status, payload, org_id)
  values ('paddle', 'evt_p94_two', 'subscription.activated', 'stored', '{}'::jsonb, v_org)
  returning id into v_event_two;

  -- The provider's subscription entity, shaped as 0187 reads it.
  v_data := jsonb_build_object(
    'items', jsonb_build_array(jsonb_build_object('price', jsonb_build_object('id', 'pri_p94'))),
  ...
  if not (v_verdict ->> 'ok')::boolean then
    raise exception 'p94.1: the activation transition refused: %', v_verdict ->> 'detail';
  end if;

  select * into v_row from private.subscription_activation_emails where org_id = v_org;
  if not found then raise exception 'p94.1: no activation email was recorded'; end if;
  if v_row.to_email <> 'p94-owner@tenant.example' then
    raise exception 'p94.1: the recipient is %, not the owner resolved server-side', v_row.to_email;
  end if;
  if v_row.status <> 'pending' or v_row.attempt_count <> 0 then
    raise exception 'p94.1: a fresh debt must be pending with no attempts, got %/%',
      v_row.status, v_row.attempt_count;
  end if;
  if v_row.plan_key <> 'p94_plan' then
    raise exception 'p94.1: the plan came from somewhere other than the price map: %', v_row.plan_key;
  end if;
  ...
  if not (v_verdict ->> 'ok')::boolean then
    raise exception 'p94.2: the second activation refused: %', v_verdict ->> 'detail';
  end if;
  select count(*) into v_count from private.subscription_activation_emails where org_id = v_org;
  if v_count <> 1 then
    raise exception 'p94.2: a redelivered activation produced % rows; exactly one is the contract',
      v_count;
  end if;
  select * into v_row from private.subscription_activation_emails where org_id = v_org;
  if v_row.billing_event_id <> v_event then
    raise exception 'p94.2: the second activation overwrote the first event reference';
  end if;
  ...
  if v_claim ->> 'state' <> 'claimed' then
    raise exception 'p94.3: the pending debt was not claimable, got %', v_claim ->> 'state';
  end if;
  if v_claim ->> 'to_email' <> 'p94-owner@tenant.example' then
    raise exception 'p94.3: the claim handed over the wrong address';
  end if;
  if v_claim ->> 'plan_label' <> 'p94 plan' then
    raise exception 'p94.3: the claim did not resolve the current plan label, got %',
      v_claim ->> 'plan_label';
  end if;
  if v_claim ->> 'locale' is null then
    raise exception 'p94.3: the claim resolved no locale, so the email has no language';
  end if;
  v_claim := service_claim_subscription_activation_email();
  if v_claim ->> 'state' <> 'idle' then
    raise exception 'p94.3: a leased row was claimed a second time (state %)', v_claim ->> 'state';
  end if;

  -- ===== 4. Settling twice is a no-op, not a second send =====
  v_settle := service_settle_subscription_activation_email(v_org, 'sent', 'resend_p94', null);
  if v_settle ->> 'state' <> 'sent' then
    raise exception 'p94.4: the settlement did not record a send';
  end if;
  v_settle := service_settle_subscription_activation_email(v_org, 'sent', 'resend_p94_again', null);
  if v_settle ->> 'state' <> 'already_sent' then
    raise exception 'p94.4: a replayed settlement was not idempotent, got %', v_settle ->> 'state';
  end if;
  select * into v_row from private.subscription_activation_emails where org_id = v_org;
  if v_row.provider_message_id <> 'resend_p94' then
    raise exception 'p94.4: a replayed settlement overwrote the provider message id';
  end if;
  if v_row.sent_at is null or v_row.lease_expires_at is not null then
    raise exception 'p94.4: a sent row must carry a timestamp and hold no lease';
  end if;
  ...
  if v_count <> 1 then
    raise exception 'p94.5: a re-activation after the welcome produced % sent rows', v_count;
  end if;
  v_claim := service_claim_subscription_activation_email();
  if v_claim ->> 'state' <> 'idle' then
    raise exception 'p94.5: a settled organization became claimable again (state %)',
      v_claim ->> 'state';
  end if;

  -- ===== 6. No active owner: no debt, and the entitlement write still succeeds =====
  insert into private.billing_events (provider, provider_event_id, event_type, status, payload, org_id)
  values ('paddle', 'evt_p94_noowner', 'subscription.activated', 'stored', '{}'::jsonb, v_org_noown)
  returning id into v_event;
  v_verdict := private.billing_apply_subscription_state(v_org_noown, v_data, v_event, 'paddle', true);
  if not (v_verdict ->> 'ok')::boolean then
    raise exception 'p94.6: a missing owner address blocked the entitlement write: %',
      v_verdict ->> 'detail';
  end if;
  if exists (select 1 from private.subscription_activation_emails where org_id = v_org_noown) then
    raise exception 'p94.6: a debt was recorded with no owner address to send it to';
  end if;
  if (select plan_key from organization_subscriptions where org_id = v_org_noown) <> 'p94_plan' then
    raise exception 'p94.6: the plan was not granted to the ownerless organization';
  end if;
  ...
  if has_function_privilege('authenticated',
    ...
    raise exception 'p94.7: a tenant role can run the activation-email commands';
  end if;

  raise notice 'p94 passed: seven cases';
end
$suite$;
