...
--
-- Each assertion below is one half of the rule 0169 installed: the newer date wins the present,
...
insert into auth.users (id, email) values
  ('2a600000-0000-4000-8000-000000000001', 'owner-p60@example.test');
...
select pg_temp.p60_assert(
  (select round(current_price, 2) = 14 and price_effective_date = date '2026-09-01'
     from public.supplier_products where id = '5a600000-0000-4000-8000-000000000001'),
  'an OLDER effective date overwrote the current price -- the defect 0169 exists to stop');
...
-- Deliberately NOT guarded by 0169: an older price list may still be the latest word on whether
