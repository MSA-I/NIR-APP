...
select pg_temp.p75_offboarding_request(
  ...
  now() - interval '8 years', 'export_ready', false, true);
-- #301 (0257): the one request in this file that keeps the hold 0103:58 puts on EVERY request and
...
-- ===== E10 -- the hold has a lift, and lifting it is what opens the gate (#301, 0257) =====
--
...
select pg_temp.p75_assert(
  ...
  'E10: offboarding.legal_hold is held by a role other than super_admin -- #301 named one');
...
-- THIS IS THE DECISION. #261 built four gates; #301 is the only thing that can open the second.
select pg_temp.p75_assert(
  ...
  'E10: lifting the hold did not clear the purge gate -- #301 exists for exactly this');
...
-- a real command and E10 above exercises it. PLACING one is not, and deliberately so: #301 ruled
