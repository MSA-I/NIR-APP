...
create function pg_temp.p60_assert(p_condition boolean, p_message text)
returns void language plpgsql as $$
begin
  if not coalesce(p_condition, false) then
    raise exception 'P60 email delivery assertion failed: %', p_message;
  end if;
end
$$;

create function pg_temp.p60_expect_error(p_sql text, p_fragment text)
returns void language plpgsql as $$
begin
  begin
    execute p_sql;
    raise exception 'P60 expected error containing %, statement succeeded: %', p_fragment, p_sql;
  exception when others then
    if sqlerrm like 'P60 expected error%' or position(p_fragment in sqlerrm) = 0 then
      raise;
    end if;
  end;
end
...
create function pg_temp.p60_actor(p_user uuid)
returns void language plpgsql as $$
begin
...
create function pg_temp.p60_service()
returns void language plpgsql as $$
begin
...
insert into public.organizations (id, name, status) values
  ('1a510000-0000-4000-8000-000000000001', 'P60 A', 'active'),
  ('1a510000-0000-4000-8000-000000000002', 'P60 B', 'active');
insert into auth.users (id, email) values
  ('2a510000-0000-4000-8000-000000000001', 'owner-a-p60@example.test'),
  ('2a510000-0000-4000-8000-000000000002', 'accountant-a-p60@example.test'),
  ('2a510000-0000-4000-8000-000000000003', 'owner-b-p60@example.test');
insert into public.profiles (id, org_id, full_name, role) values
  ('2a510000-0000-4000-8000-000000000001', '1a510000-0000-4000-8000-000000000001', 'P60 owner A', 'owner'),
  ('2a510000-0000-4000-8000-000000000002', '1a510000-0000-4000-8000-000000000001', 'P60 accountant A', 'accountant'),
  ('2a510000-0000-4000-8000-000000000003', '1a510000-0000-4000-8000-000000000002', 'P60 owner B', 'owner');
insert into public.suppliers (id, org_id, name, status, email, phone) values
  ('4a510000-0000-4000-8000-000000000001', '1a510000-0000-4000-8000-000000000001',
   'P60 supplier', 'active', 'Supplier@Example.TEST', '050-1234567'),
  ('4a510000-0000-4000-8000-000000000002', '1a510000-0000-4000-8000-000000000001',
   'P60 no-email supplier', 'active', null, null);
insert into public.products (id, org_id, name, unit) values
  ('3a510000-0000-4000-8000-000000000001', '1a510000-0000-4000-8000-000000000001', 'P60 rice', 'kg');
...
select pg_temp.p60_service();
select pg_temp.p60_assert(
...
select pg_temp.p60_actor('2a510000-0000-4000-8000-000000000002');
select pg_temp.p60_expect_error(
  $$select set_supplier_communication_preferences('4a510000-0000-4000-8000-000000000001',
    'email', 'he', null, null, false, 'x')$$,
  'not_authorized');

select pg_temp.p60_actor('2a510000-0000-4000-8000-000000000001');
select pg_temp.p60_expect_error(
  $$select set_supplier_communication_preferences('4a510000-0000-4000-8000-000000000001',
...
select set_supplier_communication_preferences(
  '4a510000-0000-4000-8000-000000000001', 'email', 'he', null, null, true, 'P60 enable email');
select pg_temp.p60_assert(
...
create table pg_temp.p60_state (key text primary key, value text);
do $$
declare
  v jsonb;
begin
  v := claim_email_order_message('6a510000-0000-4000-8000-000000000001', 'P60 first send');
  if v ->> 'state' <> 'claimed' then
    raise exception 'P60: expected claimed, got %', v ->> 'state';
  end if;
  insert into pg_temp.p60_state values
...
select pg_temp.p60_assert(
  (select status = 'sending' and attempt_count = 1 and to_email = 'supplier@example.test'
   ...
   where id = (select value::uuid from pg_temp.p60_state where key = 'message1')),
  'first claim: sending, attempt 1, normalized destination, template stamped, link attached');
select pg_temp.p60_assert(
  (select value ~ '^[0-9a-f]{64}$' from pg_temp.p60_state where key = 'token1'),
  'the claim returns the raw portal token exactly once');
select pg_temp.p60_assert(
  (select l.token_hash = encode(sha256(convert_to(
      (select value from pg_temp.p60_state where key = 'token1'), 'UTF8')), 'hex')
   from email_order_messages m
   join supplier_order_links l on l.id = m.link_id
   where m.id = (select value::uuid from pg_temp.p60_state where key = 'message1')),
...
select pg_temp.p60_expect_error(
  format($$select service_settle_email_order_message(%L, 'accepted', 'prov-1', 200, null, null)$$,
    (select value from pg_temp.p60_state where key = 'message1')),
  'service_role_required');

select pg_temp.p60_service();
select service_settle_email_order_message(
  (select value::uuid from pg_temp.p60_state where key = 'message1'),
...
select set_supplier_communication_preferences(
  ...
  'P60 add destination');
create table pg_temp.p60_state2 (key text primary key, value text);
do $$
declare
  v jsonb;
begin
  v := claim_email_order_message('6a510000-0000-4000-8000-000000000002', 'P60 second order');
  insert into pg_temp.p60_state2 values ('message2', v ->> 'message_id');
end
$$;
select pg_temp.p60_assert(
  (select to_email = 'backup@example.test' and locale = 'en'
   from email_order_messages
   where id = (select value::uuid from pg_temp.p60_state2 where key = 'message2')),
  'the destination override wins and the locale follows the preference');

select pg_temp.p60_service();
select service_settle_email_order_message(
  (select value::uuid from pg_temp.p60_state2 where key = 'message2'),
...
begin
  ...
  if v ->> 'state' <> 'claimed' or (v ->> 'attempt')::int <> 2 then
    raise exception 'P60: expected attempt 2 claim, got %', v;
  end if;
end
$$;
...
select pg_temp.p60_expect_error(
  format($$select reset_email_order_message(%L, 'x')$$,
    (select value from pg_temp.p60_state2 where key = 'message2')),
  'not_authorized');
select pg_temp.p60_actor('2a510000-0000-4000-8000-000000000001');
select reset_email_order_message(
  (select value::uuid from pg_temp.p60_state2 where key = 'message2'), 'P60 manual replay');
select pg_temp.p60_assert(
...
select pg_temp.p60_assert(
  (select count(*) = 1 from audit_logs
   where action = 'email_order_message_reset'
     and entity_id = (select value::uuid from pg_temp.p60_state2 where key = 'message2')
     and reason = 'P60 manual replay'),
  'the reset is audited');

-- ===== 6. Tenancy and the browser surface =====
select pg_temp.p60_actor('2a510000-0000-4000-8000-000000000003');
