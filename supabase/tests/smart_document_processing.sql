...
-- Build the suspended tenant while it is writable, then use the same audited, step-up protected
-- lifecycle command as production. Directly seeding status=suspended would correctly block the
-- profile/document/job fixture writes and would not prove the lifecycle boundary.
select set_config(
  'request.jwt.claim.sub', '25000000-0000-4000-8000-000000000007', false
);
...
select public.set_organization_lifecycle(
  '15000000-0000-4000-8000-000000000003', 'suspended', null,
  'P4 suspend tenant after smart document fixture setup'
);
