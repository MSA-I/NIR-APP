...
-- Exercise the provisioning regression: service_role keeps the table privileges needed to create,
-- read and modify an organization even though browser roles do not. A tenant hard-delete is no
-- longer a valid CRUD probe: the trial/offboarding latch deliberately blocks its cascading child
-- deletes until an explicit offboarding contract exists.
