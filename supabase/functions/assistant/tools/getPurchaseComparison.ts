...
//   * `saved` / `extra` come from src/lib/orderComparison.ts -- the SAME compareLine() and
//     summarizeComparison() the New Order comparison panel calls. One formula in one file: a
...
function projectSupplier(raw: Record<string, unknown>): SupplierRow | null {
  ...
  return {
    ...
    below_minimum: raw.below_minimum === true,
...
function toQuotes(line: ComparisonLine): OfferQuote[] {
  ...
  for (const offer of line.offers) {
    if (offer.unit_price === null) continue;
    quotes.push({
...
export const getPurchaseComparison: AssistantTool = {
  ...
  description:
    ...
    "האפשרות הזולה הבאה; מוצר בלי חלופה מוחזר כ־null ויש להציגו כ־`—` ולא כאפס.",
  inputSchema,
  inputJsonSchema: {
    type: "object",
    properties: {
  ...
  async run(ctx: ToolContext, input: unknown) {
    ...
    for (const line of lines) {
      ...
      const comparison = line.qty === null
        ? { status: "no_basis", savedVsNext: null, extraVsCheapest: null } as LineComparison
      ...
      facts.push(ctx.evidence.fact({
        ...
        label: `סכום השורה במחירי הרגע — ${productLabel}` +
      ...
      facts.push(ctx.evidence.fact({
        ...
        subject: { entity: "product", id: line.product_id },
        label: `${STATUS_LABELS[comparison.status] ?? comparison.status} — ${productLabel}`,
        // null, not 0: `single_offer` and `no_basis` mean there was nothing to compare against.
    ...
    facts.push(ctx.evidence.fact({
      ...
      subject: null,
      label: "סך החיסכון מול האפשרות הזולה הבאה בכל הסל",
      value: summary.saved,
      unit: "ils",
      tool: getPurchaseComparison.name,
      as_of: asOf,
    ...
    facts.push(ctx.evidence.fact({
      ...
      subject: null,
      label: "סך התשלום העודף מול האפשרות הזולה ביותר בכל הסל",
      value: summary.extra,
      unit: "ils",
      tool: getPurchaseComparison.name,
      as_of: asOf,
    ...
    const breaches = suppliers.filter((supplier) => supplier.below_minimum);
    facts.push(ctx.evidence.fact({
      kind: "metric.count",
      subject: null,
    ...
    for (const supplier of suppliers) {
      ...
      facts.push(ctx.evidence.fact({
        ...
        label: `סכום הסל אצל ${supplierLabel}`,
      ...
      if (supplier.below_minimum) {
        facts.push(ctx.evidence.fact({
          ...
          label: `חסר כדי להגיע למינימום ההזמנה של ${supplierLabel}` +
            ` (מינימום ${supplier.min_order_amount ?? "—"})`,
    ...
    return {
      data: [{
        ...
        saved_total: summary.saved,
        extra_total: summary.extra,
