...
//
//   * Dependency-free. No imports, no network, no clock, no Deno APIs. A gate that needed the
//     network to decide whether the network is allowed would be circular, and a gate that read
//     a clock would answer differently on two machines for the same evidence. Given the same
//     evidence it returns the same decision, always -- which is why the whole suite runs with
...
function refuse(unmet: GovernanceUnmetRow[]): ProviderGovernanceDecision {
  const detail = unmet.map((entry) => `${entry.row}=${entry.cause}`).join(",");
  return { allowed: false, reason: `assistant_governance_incomplete:${detail}`, unmet };
}
...
export function assertGovernedProviderConstruction(
  ...
  const rechecked = decision.allowed
    ? assertProviderGovernance({ provider: decision.provider, rows: decision.rows })
    : decision;
...
function parseEvidenceRow(raw: string | undefined): GovernanceEvidenceRow | null {
  ...
  for (const segment of raw.split(";")) {
    ...
    if (separator <= 0) return unparsable();
    const key = segment.slice(0, separator).trim().toLowerCase();
    const value = segment.slice(separator + 1).trim();
    if (fields.has(key)) return unparsable();
