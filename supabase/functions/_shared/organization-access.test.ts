...
Deno.test("organization access preflight consumes the canonical server projection", () => {
  for (const access_mode of ["active", "trial", "grace"]) {
    if (!organizationWriteAllowed({ access_mode })) throw new Error(`${access_mode} rejected`);
  }
  for (const access_mode of ["read_only", "offboarding", "suspended"]) {
