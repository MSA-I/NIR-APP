...
מתחת ל־`lg` מעטפת האפליקציה היא phone mode: כפתור תפריט בצד ימין פותח מגירה מימין ובה אותו
`NAV` שמזין את סרגל הדסקטופ. המגירה היא **מקור הניווט המלא היחיד**; סרגל הפעולות התחתון חושף
רק פעולות שכיחות ואינו רשימת יעדים מקבילה. שם הארגון הוא מיתוג בלבד; החזרה לדשבורד נעשית דרך
הפריט המפורש בתפריט. המגירה ננעלת ל־focus, נסגרת ב־Escape ומחזירה focus לטריגר. המעטפת משתמשת
ב־`100dvh`, ב־logical properties וב־safe-area; תוכן אינו רשאי להרחיב את רוחב המסמך או להסתתר מאחורי סרגל הפעולות.
...
### פעולות מהירות גלובליות
מתחת ל־`lg` הפעולות המורשות לתפקיד מופיעות בסרגל תחתון קבוע וישיר — כל פעולה זמינה בלחיצה אחת,
ללא פתיחת תפריט. הסדר נשמר ממערך `quickActions` הקנוני: “הזמנה חדשה”, “מרכז הבקרה”, “צילום מסמך”,
“קבלת סחורה”, “חשבונית חדשה”. owner ו־office מקבלים את כל החמש; kitchen מקבל אותו סדר לאחר סינון
“מרכז הבקרה”; accountant, ‏payer ו־supplier אינם מקבלים סרגל ריק. “צילום מסמך” פותח ישירות את מצלמת/בורר הקבצים
הילידיים. מגירת ההמבורגר נשארת מקור הניווט המלא — הסרגל אינו משכפל אותה.

הסרגל מכבד את `safe-area-inset-bottom`, והמעטפת שומרת עבורו מרווח תוכן כך שלא יסתיר שדות או פעולות.
הוא אינו יוצר גלילה אופקית גם ב־320px וב־zoom. כל יעד מגע הוא 44px לפחות, לכל פעולה שם נגיש,
ותווית גלויה נשברת לשתי שורות לכל היותר. במובייל אין FAB או speed-dial נוסף, ואין פס פעולות מקביל
בדשבורד.

טוסטים והודעת עדכון PWA חיים באותה ערימת overlays מעל הסרגל, ולכן נערמים אנכית גם בהגדלת טקסט
ואינם מכסים זה את זה. ב־`sm` ומעלה הודעת ה־PWA נצמדת ל־inline-start, בצד הנגדי ל־FAB הדסקטופי.

ב־`lg` ומעלה נשמר ה־speed-dial הקיים: trigger ריבועי בגודל 48px (`h-12 w-12`, ‏`bg-action`, ‏`--shadow-fab`)
פותח את אותן פעולות ובאותו סדר. הוא תומך ב־roles ‏menu/menuitem, חיצים/Home/End/Escape, החזרת focus
