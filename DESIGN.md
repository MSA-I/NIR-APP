...
- **חריג עומק בשם — dashboard-depth:** בתוך מרכז הבקרה בלבד, כרטיסים מקבלים
  `shadow-dashboard`, צל פטרול מעט עמוק יותר שמפריד בין חמשת אזורי ההחלטה. הסלקטור חייב להישאר
...
אייקון סמנטי בתוך chip שקט · תווית · ערך סופי מיידי במונו · הקשר תקופה · אזור מגמה בגובה קבוע.
"נרכש" ו"שולם" רשאים להציג ספארקליין ליניארי של שמונה שבועות ו־delta מול אותם ימים בחודש הקודם;
הם מוצגים רק כשיש בסיס השוואה אמיתי ולפחות שני שבועות שבהם נרשמה פעילות. "יתרת חשבוניות" מציגה
את מספר החשבוניות הפתוחות בלבד — לעולם לא מגמה מומצאת. ‏AttentionZone נשאר רכיב החתימה — ללא
שינוי מבני. כל קישור בפס ובשורות ההחלטה הוא יעד מגע של 44px לפחות, עם focus-visible ו־active גלויים.
...
ספארקליין הוא הקשר משני, לא KPI נוסף: שמונה buckets שבועיים נושאים `label`, סכום ומספר רשומות כדי
להבדיל בין אפס מדוד לבין bucket מלאכותי. ‏gradient ID חייב להיות ייחודי ו־ARIA מתאר את הסדרה.
גרף מלא או ספארקליין נכנסים פעם אחת בלבד בחשיפה הראשונה ל־viewport; רענון נתונים אינו מפעיל את
האנימציה מחדש. אם `IntersectionObserver` חסר, או תחת `prefers-reduced-motion`, הגרף מוצג סטטי מיד.
גרף שכל ערכיו הם buckets ריקים מוחלף ב־empty state.

### Motion budget — דשבורד
חמשת אזורי ההחלטה נכנסים פעם אחת כשהשלד מוחלף בנתונים: 350ms, תזוזה של 7px ו־60ms בין אזורים.
חותמת "עודכן ב־" מתחלפת ב־fade של 180ms עם `aria-live="polite"`. אנימציית הגרפים מוגבלת לכניסה
הראשונה ל־viewport. אין count-up לסכומים, אין נורית LIVE, אין הבהוב מחזורי ואין אנימציית hover
