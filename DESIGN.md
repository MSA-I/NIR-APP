...
**רישום z-index:** בר קבלה 30 · סרגל צד/כותרות/FAB ‏40 · מגירה/מודאל 50 · טוסט 60.
...
מתחת ל־`lg` מעטפת האפליקציה היא phone mode: סרגל תחתון תפקידי חושף את יעדי היום־יום, וכפתור
„עוד” פותח מגירה מימין עם היעדים המורשים שאינם כבר בסרגל. שני המשטחים נגזרים מאותו קטלוג
הרשאות; המגירה אינה משכפלת יעדים. צילום מסמך הוא פקודה מרכזית ולא route. שם הארגון הוא זהות
...
בסרגל הדסקטופ יעדי העבודה היומיים גלויים; „ניהול” ו„בקרה” הם disclosure מקופל, והגדרות owner
נשארות באזור תחתון נפרד. `NAV_SECTIONS` הוא קטלוג המסלולים וההרשאות, לא תפריט שמציג תמיד את כל
מבנה המערכת. `isRouteFamilyActive` משותף לסרגל, למגירה ולמובייל כדי שפרטי רשומה ישמרו את הקשרם.
...
### ניווט מובייל ופעולות מהירות
מתחת ל־`lg` מוצגים עד חמישה יעדי יום־יום לפי תפקיד. owner/office מדגישים הזמנות וחשבוניות;
kitchen מדגיש קבלה ומסמכים; accountant מדגיש חשבוניות, ביצוע ובנק; payer ו־supplier מקבלים רק את
המעט הדרוש להם. לתפקידים תפעוליים צילום מסמך יושב במרכז ופותח את מצלמת/בורר הקבצים הילידיים.
„עוד” פותח את יתר היעדים ואינו מקבל `aria-current` כשהיעד הפעיל כבר גלוי בסרגל.

הסרגל מכבד את `safe-area-inset-bottom`, והמעטפת שומרת עבורו מרווח תוכן כך שלא יסתיר שדות או פעולות.
הסרגל אינו יוצר גלילה אופקית גם ב־320px וב־zoom. כל יעד מגע הוא 44px לפחות, לכל פעולה שם נגיש,
ותווית גלויה נשברת לשתי שורות לכל היותר. במסלולי focus ניווט מתחרה מוסר; בקבלת סחורה נשארת פקודת
הצילום ובר המשימה תופס את השטח הקבוע. במובייל אין FAB או speed-dial נוסף.
...
ומעלה הודעת ה־PWA נצמדת ל־inline-start, בצד הנגדי ל־FAB הדסקטופי.

ב־`lg` ומעלה נשמר speed-dial של **פקודות בלבד**: הזמנה חדשה, צילום מסמך וחשבונית חדשה לפי הרשאה.
ניווט שייך לסרגל. ה־trigger ריבועי בגודל 48px (`h-12 w-12`, ‏`bg-action`, ‏`--shadow-fab`) ותומך
ב־roles ‏menu/menuitem, חיצים/Home/End/Escape, החזרת focus
