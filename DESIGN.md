...
מתחת ל־`lg` מעטפת האפליקציה היא phone mode עם **משטח ניווט ראשי אחד בלבד**: כפתור תפריט בצד ימין
פותח מגירה מימין ובה אותו `NAV` שמזין את סרגל הדסקטופ. אין ניווט תחתון ואין רשימת יעדים מקבילה.
שם הארגון הוא מיתוג בלבד; החזרה לדשבורד נעשית דרך הפריט המפורש בתפריט. המגירה ננעלת ל־focus,
נסגרת ב־Escape ומחזירה focus לטריגר. המעטפת משתמשת ב־`100dvh`, ב־logical properties וב־safe-area;
תוכן אינו רשאי להרחיב את רוחב המסמך.
...
כפתור ריבועי בגודל 48px (`h-12 w-12`, ‏`bg-action`, ‏`--shadow-fab`) פותח speed-dial של הפעולות
התכופות המורשות לתפקיד, ובהן צילום מסמך. הוא מופיע גם בדשבורד; במסכים ממוקדים שכבר מחזיקים פעולה
ראשית — `/orders/new`, ‏`/invoices/new`, ‏`/receiving/:orderId` — הוא מוסתר כדי שלא יתחרה בה.
מיקומו שמור בקצה הלוגי של הכותרת העליונה ומכבד safe-area; הכותרת מפנה לו רוחב במובייל כך שהטריגר
אינו יושב מעל KPI, פעולה או מידע. הוא מופיע פעם אחת ב־DOM לפני התוכן, ולכן סדר המקלדת תואם למיקום
החזותי. התפריט נפתח כלפי מטה, מוגבל ל־viewport וניתן לגלילה גם ב־320px וב־zoom. הטריגר והפריטים הם יעדי מגע
של 44px לפחות, עם roles ‏menu/menuitem, שמות נגישים, חיצים/Home/End/Escape, החזרת focus לטריגר,
וסגירה בלחיצה בחוץ או במעבר route. אין פס פעולות מקביל בדשבורד.

הפתיחה משתמשת ב־opacity ובתזוזת block קטנה: 140ms ‏ease-out עם stagger של 15ms, כך שגם חמישה
פריטים מסיימים בתוך 200ms. סימן הפלוס מסתובב ל־X במשך 180ms. תחת `prefers-reduced-motion`
האנימציה והמעבר מבוטלים והמצב הסופי מוצג מיד.
