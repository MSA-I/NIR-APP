...
### הפעולה הגלובלית לצילום
כפתור מצלמה ריבועי בגודל 48px (`h-12 w-12`, ‏`bg-action`, ‏`--shadow-fab`) הוא הפעולה הגלובלית
היחידה. הוא מופיע גם בדשבורד, פותח צילום מסמך ישירות ואינו נפתח לתפריט. במסכים ממוקדים שכבר מחזיקים
פעולה ראשית — `/orders/new`, ‏`/invoices/new`, ‏`/receiving/:orderId` — הוא מוסתר כדי שלא יתחרה בה.
מיקומו מכבד safe-area במובייל ונשמר במרווח 2rem מהקצוות בדסקטופ. אין פס פעולות בדשבורד,
אין speed-dial ואין רשימת קיצורי דרך המשכפלת את הניווט.
