...
(סקירת קוד, 10.08.2026): שער ה-build ("אין טסטים" מול תשעה סקריפטי `check:*` ומאות בדיקות),
מנגנון היתרות (views מול פונקציות `SECURITY DEFINER` מ-`0022`), גרסת React Router, והיעדר
`DEBT-REGISTER` ושער האיכות. עותק שני של חוקה הוא בדיוק מנגנון הסטייה ש-`check:counts` נבנה
נגדו — ולכן אין כאן עותק, יש מצביע.
